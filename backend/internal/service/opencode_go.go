...
// @project-doc docs/interfaces/opencode_upstream.md#opencode_accounts_and_protocols
...
const (
	...
	// DefaultOpenCodeGoTestModel 是未指定测试模型时使用的 Chat 模型。
...
// DefaultOpenCodeGoProtocolRules 返回 GO 的内置模型协议规则。
...
func normalizeOpenCodeCredentials(account *Account, isCreate bool) error {
	if account.Type != AccountTypeAPIKey {
		return infraerrors.BadRequest("OPENCODE_ACCOUNT_TYPE_INVALID", "OpenCode accounts must use API Key credentials")
	}
	if account.Credentials == nil {
		account.Credentials = make(map[string]any)
	}
	for _, field := range []struct{ key, fallback string }{{"account_mode", AccountModeGo}, {"api_protocol", APIProtocolAdaptive}} {
		raw, exists := account.Credentials[field.key]
		value, valid := raw.(string)
		if exists && raw != nil && !valid {
			return infraerrors.BadRequest("OPENCODE_CREDENTIALS_INVALID", field.key+" must be a string")
		}
		value = strings.TrimSpace(value)
		if value == "" {
			value = field.fallback
		}
		if field.key == "account_mode" && value != AccountModeZen && value != AccountModeGo {
			return infraerrors.BadRequest("OPENCODE_ACCOUNT_MODE_INVALID", "account_mode must be zen or go")
		}
		if field.key == "api_protocol" && value != APIProtocolAdaptive && !isNativeOpenCodeGoProtocol(value) {
			return infraerrors.BadRequest("OPENCODE_PROTOCOL_INVALID", "api_protocol is unsupported")
		}
		if exists || isCreate {
			account.Credentials[field.key] = value
		}
	}
	return NormalizeOpenCodeGoProtocolRulesCredentials(account.Credentials)
}

// openCodeProtocolBaseURL 保留显式中继主机与路径，只在未配置时选用账号模式的默认端点。
func (a *Account) openCodeProtocolBaseURL(protocol string) string {
	if a.IsAdaptiveAPIProtocol() {
		if bases, ok := a.Credentials["api_base_urls"].(map[string]any); ok {
			if base, ok := bases[protocol].(string); ok && strings.TrimSpace(base) != "" {
...
// resolveOpenCodeGoMappedModel 仅为选协议观察一次映射结果，实际 body 由原有转发器改写一次。
func resolveOpenCodeGoMappedModel(account *Account, body []byte, defaultMappedModel string) string {
	...
	return normalizeOpenAIModelForUpstream(account, resolveOpenAIForwardModel(account, original, defaultMappedModel))
}

// stampOpenAIResponsesUpstreamEndpoint 记录实际 Responses 端点，兼容错误日志和成功用量记录。
func stampOpenAIResponsesUpstreamEndpoint(c *gin.Context, result *OpenAIForwardResult) {
	endpoint := GetActualOpenAIUpstreamEndpoint(c)
	if endpoint == "" {
