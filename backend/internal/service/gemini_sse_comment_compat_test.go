...
func TestAntigravityGeminiStreamKeepsCommentKeepaliveForOrdinaryClients(t *testing.T) {
	// 首次 tick 可能紧邻上游数据而被忽略，用虚拟时间覆盖第二次 tick，避免调度抖动。
	synctest.Test(t, func(t *testing.T) {
		out := runAntigravityGeminiStreamWithIdle(t, "curl/8.7.1", 2100*time.Millisecond)
...
func TestAntigravityGeminiStreamSkipsCommentKeepaliveForGoGenai(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		out := runAntigravityGeminiStreamWithIdle(t, "google-genai-sdk/1.71.0 gl-go/go1.28-20260721-RC03", 2100*time.Millisecond)
		require.Contains(t, out, `"text":"partial"`)
...
// 上游独立注释帧和 data 帧中的注释都必须过滤，且不能留下空事件；
// 普通客户端继续收到原有注释和帧分隔，不受 SDK 兼容逻辑影响。
func TestAntigravityGeminiStreamFiltersUpstreamCommentsForSDKs(t *testing.T) {
	const dataFrame = "data: {\"candidates\":[]}\n\n"
	cases := []struct {
		name       string
		header     string
		clientHint string
		...
		{"go user agent", "User-Agent", "google-genai-sdk/1.71.0 gl-go/go1.28", dataFrame},
		{"python api client", "X-Goog-Api-Client", "google-genai-sdk/1.20.0 gl-python/3.12.4", dataFrame},
		{"ordinary client", "User-Agent", "curl/8.7.1", ": before\n\n" + dataFrame + ": mixed\n\n: after\n\n"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			for _, newline := range []string{"\n", "\r\n"} {
				...
				c, recorder := newAntigravityCompatContext(http.MethodPost, "/v1beta/models/gemini-3.8-flash:streamGenerateContent", nil)
				c.Request.Header.Set(tc.header, tc.clientHint)
				stream := strings.Join([]string{
					": before", "", `data: {"response":{"candidates":[]}}`, ": mixed", "", ": after", "", "",
				}, newline)
				resp := &http.Response{StatusCode: http.StatusOK, Header: http.Header{}, Body: io.NopCloser(strings.NewReader(stream))}
				_, err := svc.handleGeminiStreamingResponse(c, resp, time.Now())
				require.NoError(t, err)
				require.Equal(t, tc.want, recorder.Body.String(), "newline %q must not leave empty frames", newline)
			}
		})
	}
}
