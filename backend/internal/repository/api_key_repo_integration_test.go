...
func (s *APIKeyRepoSuite) TestSmartRoutingKeyPersistenceAndGroupInvalidation() {
	user := s.mustCreateUser("smart-routing@test.com")
	first := s.mustCreateGroup("smart-first")
	second := s.mustCreateGroup("smart-second")
	key := &service.APIKey{
		UserID: user.ID, Key: "sk-smart-routing", Name: "Smart Routing", Status: service.StatusActive,
		SmartRouting: true,
		CompositeGroups: []service.APIKeyCompositeGroup{
			{GroupID: second.ID, Prefix: "route_second", NormalizedPrefix: "route_second", SortOrder: 0},
			{GroupID: first.ID, Prefix: "route_first", NormalizedPrefix: "route_first", SortOrder: 1},
		},
	}
	// 创建、列表和认证查询必须还原同一模式与顺序，分组失效查询也必须包含智能路由 Key。
	s.Require().NoError(s.repo.Create(s.ctx, key))
	got, err := s.repo.GetByKeyForAuth(s.ctx, key.Key)
	s.Require().NoError(err)
	s.Require().True(got.SmartRouting)
	s.Require().False(got.IsComposite)
	s.Require().Equal([]int64{second.ID, first.ID}, got.SmartRoutingGroupIDs())
	count, err := s.repo.CountByGroupID(s.ctx, first.ID)
	s.Require().NoError(err)
	s.Require().Equal(int64(1), count)

	got.CompositeGroups[0], got.CompositeGroups[1] = got.CompositeGroups[1], got.CompositeGroups[0]
	got.CompositeGroups[0].SortOrder = 0
	got.CompositeGroups[1].SortOrder = 1
	s.Require().NoError(s.repo.Update(s.ctx, got, service.APIKeyUpdateFields{CompositeConfiguration: true}))
	updated, err := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err)
	s.Require().Equal([]int64{first.ID, second.ID}, updated.SmartRoutingGroupIDs())
	_, err = s.repo.ClearGroupIDByGroupID(s.ctx, first.ID)
	s.Require().NoError(err)
	updated, err = s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err)
	s.Require().Equal([]int64{second.ID}, updated.SmartRoutingGroupIDs())

	// 转回普通模式与清理候选必须同事务更新，满足数据库互斥约束。
	updated.SmartRouting = false
	updated.GroupID = &second.ID
	updated.CompositeGroups = nil
	s.Require().NoError(s.repo.Update(s.ctx, updated, service.APIKeyUpdateFields{CompositeConfiguration: true, GroupID: true}))
	ordinary, err := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err)
	s.Require().False(ordinary.SmartRouting)
	s.Require().Empty(ordinary.CompositeGroups)
}

func (s *APIKeyRepoSuite) TestSmartRoutingCooldownPersistenceAndIndependentUpdate() {
	user := s.mustCreateUser("smart-cooldown@test.com")
	group := s.mustCreateGroup("smart-cooldown")
	key := &service.APIKey{
		UserID: user.ID, Key: "sk-smart-cooldown", Name: "Cooldown", Status: service.StatusActive,
		SmartRouting: true, SmartRoutingCooldownSeconds: 60,
		CompositeGroups: []service.APIKeyCompositeGroup{{GroupID: group.ID, Prefix: "route", NormalizedPrefix: "route", SortOrder: 0}},
	}
	s.Require().NoError(s.repo.Create(s.ctx, key))
	loaded, err := s.repo.GetByKeyForAuth(s.ctx, key.Key)
	s.Require().NoError(err)
	s.Require().Equal(60, loaded.SmartRoutingCooldownSeconds)
	bindingID := loaded.CompositeGroups[0].ID
	// 同时产生真实用量，再用旧快照改冷却，不能覆盖累计或重建关联 ID。
	_, err = s.repo.IncrementQuotaUsed(s.ctx, key.ID, 15)
	s.Require().NoError(err)
	s.Require().NoError(s.repo.IncrementRateLimitUsage(s.ctx, key.ID, 7))
	loaded.SmartRoutingCooldownSeconds = 0
	s.Require().NoError(s.repo.Update(s.ctx, loaded, service.APIKeyUpdateFields{SmartRoutingCooldown: true}))
	updated, err := s.repo.GetByID(s.ctx, key.ID)
	s.Require().NoError(err)
	s.Require().Zero(updated.SmartRoutingCooldownSeconds)
	s.Require().Equal(bindingID, updated.CompositeGroups[0].ID)
	s.Require().InDelta(15, updated.QuotaUsed, 1e-9)
	s.Require().InDelta(7, updated.Usage5h, 1e-9)
	s.Require().True(updated.SmartRouting)
	// 未声明冷却字段时，即便传入的快照带旧值也必须保留数据库当前值。
	loaded.SmartRoutingCooldownSeconds = 60
	loaded.Name = "Renamed"
	s.Require().NoError(s.repo.Update(s.ctx, loaded, service.APIKeyUpdateFields{Name: true}))
	updated, err = s.repo.GetByKeyForAuth(s.ctx, key.Key)
	s.Require().NoError(err)
	s.Require().Zero(updated.SmartRoutingCooldownSeconds)
	listed, _, err := s.repo.ListByUserID(s.ctx, user.ID, pagination.PaginationParams{Page: 1, PageSize: 20}, service.APIKeyListFilters{})
	s.Require().NoError(err)
	s.Require().Len(listed, 1)
	s.Require().Zero(listed[0].SmartRoutingCooldownSeconds)
}
