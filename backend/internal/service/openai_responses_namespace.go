...
// 保留工具调用项上的 namespace。
//
// 上游对这个字段有两套互斥要求，先判断出口与端点，API Key 再核对工具声明：
...
//     namespace 工具时（含 Lite 的 additional_tools 输入项），必须保留调用项上的
...
func hasOpenAIResponsesNamespaceToolDeclaration(body []byte) bool {
	...
	// Lite 把工具声明放入独立历史项；只检查该载体，不能误认正文内的同名字段。
	...
	input.ForEach(func(_, item gjson.Result) bool {
		if strings.EqualFold(strings.TrimSpace(item.Get("type").String()), "additional_tools") && hasNamespaceTool(item.Get("tools")) {
			found = true
			return false
		}
		return true
