...
// 备份记录与归档水位由跨实例写锁保护，手动备份和恢复更新也使用相同边界。
// Redis 故障时不能另取数据库锁，否则会与仍持有 Redis 锁的实例并行写入。
func (s *BackupService) lockBackupRecordUpdates(ctx context.Context) (context.Context, func(), error) {
	...
	for {
		...
		if s.lockCache != nil {
			...
			// 会话级建议锁占用一条连接，仓储写入还需另一条；生产环境优先使用 Redis。
...
type BackupMonthlyArchiveConfig struct {
	...
	RetainCount     int   `json:"retain_count"` // 0 表示永久保留，正数为归档池数量上限。
}

type BackupMonthlyArchive struct {
	Dates       []string `json:"dates"`        // 当前备份覆盖的归档日期，已去重。
	RetainCount int      `json:"retain_count"` // 0 为永久保留，不随以后配置被隐式降级。
}
...
// assignMonthlyArchive 在记录写锁内为已完成的定时备份分配月度归档。
// 即便没有到期归档日也推进持久水位；删除记录或降低保留数量不会回退水位。
// @project-doc docs/operations/observability_and_data_lifecycle.md#backup_monthly_archives
