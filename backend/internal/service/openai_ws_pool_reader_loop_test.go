...
func (d *openAIWSReaderLoopFakeDialer) Dial(context.Context, string, http.Header, string, *tlsfingerprint.Profile) (openAIWSClientConn, int, http.Header, error) {
...
// 无缓冲交接屏障让帧已登记但无法入队，复现 len(channel)==0 时仍有旧帧等待投递。
func TestOpenAIWSConnReaderLoop_RegisteredFrameRejectsLeaseBeforeDelivery(t *testing.T) {
	conn := newOpenAIWSConn("registered-before-delivery", 1, &openAIWSFakeConn{}, nil, nil, "")
	defer conn.close()
	conn.readerLoopResults = make(chan []byte)
	published := make(chan bool, 1)
	go func() { published <- conn.publishReaderLoopMessage([]byte(`{"type":"response.completed"}`)) }()
	require.Eventually(t, conn.readerLoopPending, time.Second, time.Millisecond)
	require.Zero(t, len(conn.readerLoopResults), "屏障前队列为空，旧的 len 检查会错误借出")
	require.False(t, conn.tryAcquire(), "已经登记的上一轮帧不能交给新租约")
...
// reader 在空闲时开始阻塞读，之后借出、发送的新请求响应仍属于新租约。
func TestOpenAIWSConnReaderLoop_IdleReadDeliversNewLeaseResponse(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("idle-read-new-response", 1, fake, nil, nil, "")
	defer conn.close()
	...
	want := []byte(`{"type":"response.completed","response":{"id":"new-turn"}}`)
	fake.messages <- want
	got, err := conn.readMessageWithTimeout(time.Second)
	require.NoError(t, err)
	require.Equal(t, want, got)
	require.False(t, conn.isUnusable(), "不能按 Read 开始时尚未借出就误判新响应")
	require.False(t, conn.readerLoopPending())
	conn.release()
}

func TestOpenAIWSConnReaderLoop_NotStartedForPlainConn(t *testing.T) {
	conn := newOpenAIWSConn("rl_plain", 1, &openAIWSFakeConn{}, nil, nil, "")
	defer conn.close()
...
func TestOpenAIWSConnReaderLoop_KeepsReaderWhileIdleSoPingSucceeds(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_idle_ping", 1, fake, nil, nil, "")
...
func TestOpenAIWSConnReaderLoop_DeliversMessagesInOrder(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_order", 1, fake, nil, nil, "")
	defer conn.close()
	require.True(t, conn.tryAcquire())
...
func TestOpenAIWSConnReaderLoop_ReadTimeoutClosesConn(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_timeout", 1, fake, nil, nil, "")
	require.True(t, conn.tryAcquire())
...
func TestOpenAIWSConnReaderLoop_UpstreamCloseWhileIdleClosesConn(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_idle_close", 1, fake, nil, nil, "")
...
func TestOpenAIWSConnReaderLoop_LocalCloseIsNotPeerClose(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_local_close", 1, fake, nil, nil, "")
	require.Eventually(t, func() bool { return fake.readers.Load() == 1 }, time.Second, 5*time.Millisecond)
...
func TestOpenAIWSConnReaderLoop_UpstreamCloseWhileReadingReturnsCause(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_read_close", 1, fake, nil, nil, "")
	require.True(t, conn.tryAcquire())
...
func TestOpenAIWSConnReaderLoop_ReadAfterUpstreamCloseReturnsCause(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_late_read", 1, fake, nil, nil, "")
	require.True(t, conn.tryAcquire())
...
func TestOpenAIWSConnReaderLoop_BufferedMessageDeliveredAfterPeerClose(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_buffered_close", 1, fake, nil, nil, "")
	require.True(t, conn.tryAcquire())
...
func TestOpenAIWSConnPool_BackgroundPingSweepToleratesSlowPong(t *testing.T) {
	...
	conn := newOpenAIWSConn("rl_slow_pong", 309, fake, nil, nil, "")
	defer conn.close()
...
func TestOpenAIWSConnPool_BackgroundPingSweepSkipsLeasedConnOnFailure(t *testing.T) {
	...
	conn := newOpenAIWSConn("rl_leased_during_ping", 310, fake, nil, nil, "")
	defer conn.close()
...
func TestOpenAIWSConnPool_BackgroundPingSweepNeverClosesConnLeasedInEvictWindow(t *testing.T) {
	...
	fake.pingHold = make(chan struct{})
	fake.pingErr = errors.New("pong lost")
	conn := newOpenAIWSConn("rl_evict_window", 312, fake, nil, nil, "")
	defer conn.close()
...
func TestOpenAIWSConnReaderLoop_ReadTimeoutAbortsWithoutCloseHandshake(t *testing.T) {
	...
	require.NoError(t, err)
	conn := newOpenAIWSConn("rl_read_timeout_abort", 1, ws, nil, nil, "")
	defer conn.close()
	require.True(t, conn.tryAcquire())
...
func TestOpenAIWSConnPingDoesNotBlockWrite(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	fake.pingHold = make(chan struct{})
	conn := newOpenAIWSConn("rl_ping_write", 1, fake, nil, nil, "")
	defer conn.close()
...
func TestOpenAIWSConnReaderLoop_DataWhileIdleRejectsLease(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_dirty_try", 1, fake, nil, nil, "")
...
func TestOpenAIWSConnReaderLoop_DataWhileIdleRejectsBlockingAcquire(t *testing.T) {
	fake := newOpenAIWSReaderLoopFakeConn()
	conn := newOpenAIWSConn("rl_dirty_acquire", 1, fake, nil, nil, "")
...
func TestOpenAIWSConnPool_ReaderLoopConnNotIdleRecycled(t *testing.T) {
	...
	conn := newOpenAIWSConn("rl_no_recycle", accountID, newOpenAIWSReaderLoopFakeConn(), nil, nil, "")
	defer conn.close()
...
func TestOpenAIWSConnReaderLoop_RealConnAnswersServerPingWhileIdle(t *testing.T) {
	...
	ws, _, _, err := newDefaultOpenAIWSClientDialer().Dial(context.Background(), "ws"+strings.TrimPrefix(server.URL, "http"), nil, "", nil)
	require.NoError(t, err)
	conn := newOpenAIWSConn("rl_real", 1, ws, nil, nil, "")
	defer conn.close()
	require.True(t, conn.hasReaderLoop())
