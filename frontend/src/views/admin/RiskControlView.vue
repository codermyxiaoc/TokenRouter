            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
              <thead class="bg-gray-50 dark:bg-dark-800">
                <tr>
                  ...
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.table.group') }}</th>
                  ...
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.table.result') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.table.highest') }}</th>
                  ...
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.table.latency') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.table.input') }}</th>
                </tr>
              </thead>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-dark-700">
              <thead class="bg-gray-50 dark:bg-dark-800">
                <tr>
                  ...
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.table.actionMeta') }}</th>
                  <th class="px-5 py-3 text-left text-xs font-medium tracking-wider text-gray-500 dark:text-gray-400">{{ t('admin.riskControl.cyberWarning') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-100 bg-white dark:divide-dark-800 dark:bg-dark-800">
                <tr v-if="logsLoading">
      ...
      <BaseDialog :show="settingsOpen" :title="t('admin.riskControl.settingsTitle')" width="extra-wide" @close="settingsOpen = false">
        <div class="space-y-6">
          ...
          <div v-else-if="activeSettingsTab === 'keywords'" class="space-y-5">
            <div class="flex items-start gap-3 rounded-lg border p-4" :class="keywordNotice.toneClass">
              <Icon :name="keywordNotice.icon" size="md" :class="keywordNotice.iconClass" />
...
type SettingsTab = 'basic' | 'scope' | 'runtime' | 'response' | 'riskThresholds' | 'keywords' | 'retention'
...
type EngineDraft = Pick<typeof configForm, typeof engineFields[number]> & { pendingDeletes: string[]; metadata: Record<string, APIKeyMetadataDraft> }
const engineDrafts = ref<Partial<Record<ModerationEngine, EngineDraft>>>({})

function captureEngineDraft(): EngineDraft {
  return structuredClone({ ...Object.fromEntries(engineFields.map(key => [key, toRaw(configForm)[key]])), pendingDeletes: [...pendingDeleteApiKeyHashes.value], metadata: toRaw(apiKeyMetadataDrafts) }) as EngineDraft
}
...
function switchEngine(value: string | number | boolean | null) {
  ...
  if (draft) {
    const { pendingDeletes, metadata, ...fields } = structuredClone(toRaw(draft))
    Object.assign(configForm, fields)
    pendingDeleteApiKeyHashes.value = pendingDeletes
    for (const hash of Object.keys(apiKeyMetadataDrafts)) delete apiKeyMetadataDrafts[hash]
    Object.assign(apiKeyMetadataDrafts, metadata)
  }
...
function engineDraftFromConfig(config: ContentModerationConfig | undefined, engine: ModerationEngine): EngineDraft {
  return {
    ...
    metadata: Object.fromEntries((config?.api_key_statuses ?? []).map(item => [item.key_hash, { priority: item.priority ?? 100, note: item.note ?? '' }])),
...
function engineDraftPayload(draft: EngineDraft): UpdateModerationEngineConfig {
  ...
  return {
    ...
    clear_api_key: draft.clear_api_key, api_key_entries: keys.length ? parseApiKeyEntries(draft.api_keys_text) : undefined,
    api_key_updates: !draft.clear_api_key && draft.api_keys_mode !== 'replace' ? Object.entries(draft.metadata).filter(([hash]) => !draft.pendingDeletes.includes(hash)).map(([key_hash, metadata]) => ({ key_hash, priority: normalizeAPIKeyPriority(metadata.priority), note: metadata.note.trim().slice(0, 200) })) : undefined,
...
const keywordNoticeTones = {
  ...
  warning: {
    ...
    toneClass: 'border-amber-200 bg-amber-50/80 dark:border-amber-900/50 dark:bg-amber-900/10',
...
function applyConfig(config: ContentModerationConfig) {
  ...
  const { pendingDeletes, metadata, ...fields } = structuredClone(toRaw(engineDrafts.value[configForm.engine]!))
  Object.assign(configForm, fields)
  pendingDeleteApiKeyHashes.value = [...pendingDeletes]
  for (const hash of Object.keys(apiKeyMetadataDrafts)) delete apiKeyMetadataDrafts[hash]
  Object.assign(apiKeyMetadataDrafts, metadata)
...
function parseBlockedKeywords(value: string): string[] {
  ...
  for (const line of value.split(/\r?\n/)) {
    ...
    if (seen.has(key)) continue
    seen.add(key)
    out.push(kw.slice(0, 200))
    if (out.length >= blockedKeywordMax) break
  }
  return out
}
