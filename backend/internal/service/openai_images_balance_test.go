...
// fork 的图片转发统一经过 TLS 指纹入口，测试替身保留同一响应语义。
func (u openAIImagesBalanceUpstream) DoWithTLS(r *http.Request, proxy string, accountID int64, concurrency int, _ *tlsfingerprint.Profile) (*http.Response, error) {
	return u.Do(r, proxy, accountID, concurrency)
}
