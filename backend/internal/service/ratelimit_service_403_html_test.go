...
// TestHandleUpstreamError_OpenAIHTML403DoesNotPenalizeAccount 验证常见 HTML 外形均不处罚账号。
...
// TestHandleUpstreamError_OpenAIStructured403StillPenalizes 保护真实账号级 403 的既有处罚链路。
...
func TestHandleUpstreamError_OpenAICloudflare1010DoesNotPenalizeAccount(t *testing.T) {
	for _, platform := range []string{PlatformOpenAI, PlatformOpenCodeGo} {
		t.Run(platform, func(t *testing.T) {
			repo := &rateLimitAccountRepoStub{}
			counter := &countingOpenAI403CounterCache{
				openAI403CounterCacheStub: openAI403CounterCacheStub{counts: []int64{int64(openAI403DisableThresholdDefault)}},
			}
			blocker := &runtimeBlockRecorder{}
			svc := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
			svc.SetOpenAI403CounterCache(counter)
