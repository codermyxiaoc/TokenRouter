...
func TestBackupRecovery_RestartPreservesPeerArchive(t *testing.T) {
	...
	writer := newTestBackupService(t, repo, &mockDumper{}, store)
	...
	restarting := newTestBackupService(t, repo, &mockDumper{}, store)
	...
	// 恢复扫描从读取到决策都持有写锁；其他实例只能释放后完成，近期操作仍保持运行。
...
func TestBackupRecovery_RestoreRejectsRemovedArchive(t *testing.T) {
	...
	seedStorageS3Config(t, repo)
	store := newMockObjectStore()
	cleaner := newTestBackupService(t, repo, &mockDumper{}, store)
	...
	restoring := newTestBackupService(t, repo, &mockDumper{}, store)
	delayedRepo := &backupDelayedReadRepo{mockSettingRepo: repo, loaded: make(chan struct{}), resume: make(chan struct{})}
...
func TestBackupRecovery_RestoreReservationProtectsFromPeerCleanup(t *testing.T) {
	...
	store := newMockObjectStore()
	svc := newTestBackupService(t, repo, &mockDumper{}, store)
	peer := newTestBackupService(t, repo, &mockDumper{}, store)
	cache := &fakeLeaderLockCache{}
