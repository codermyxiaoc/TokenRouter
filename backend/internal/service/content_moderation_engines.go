...
func (cfg *ContentModerationConfig) engineProfile(engine string) *ContentModerationEngineConfig {
	...
	if engine == ContentModerationEngineTypeSafe {
		...
		p = ContentModerationEngineConfig{BaseURL: cfg.BaseURL, Model: cfg.Model, ProxyID: cfg.ProxyID, APIKeys: cfg.apiKeys(), TimeoutMS: cfg.TimeoutMS, RetryCount: cfg.RetryCount, Thresholds: cfg.Thresholds, APIKeyMetadata: cfg.APIKeyMetadata}
	}
	p.APIKeyMetadata = append([]ContentModerationAPIKeyMetadata(nil), p.APIKeyMetadata...)
...
func (cfg *ContentModerationConfig) applyEngineProfile(p *ContentModerationEngineConfig) {
	cfg.engineProfileApplied = true
	cfg.BaseURL, cfg.Model, cfg.ProxyID = p.BaseURL, p.Model, cloneInt64Ptr(p.ProxyID)
	cfg.APIKeyMetadata = append([]ContentModerationAPIKeyMetadata(nil), p.APIKeyMetadata...)
...
func (s *ContentModerationService) updateEngineProfile(ctx context.Context, cfg *ContentModerationConfig, engine string, input UpdateContentModerationEngineInput) error {
	...
	if input.Thresholds != nil {
		check.Thresholds = mergeContentModerationThresholds(ContentModerationDefaultThresholds(), *input.Thresholds)
	}
	if input.ClearAPIKey {
		check.APIKey = ""
		check.APIKeys = []string{}
		check.APIKeyMetadata = []ContentModerationAPIKeyMetadata{}
	} else {
		...
		if input.DeleteAPIKeyHashes != nil && apiKeysMode != contentModerationAPIKeysModeReplace {
			check.APIKeys = deleteModerationAPIKeysByHash(check.apiKeys(), *input.DeleteAPIKeyHashes, engine)
			check.APIKey = ""
		}
		...
		if input.APIKeys != nil || input.APIKeyEntries != nil {
			if apiKeysMode == contentModerationAPIKeysModeReplace {
				check.APIKeys = normalizeModerationAPIKeys(newKeys)
				check.APIKeyMetadata = []ContentModerationAPIKeyMetadata{}
			} else {
				check.APIKeys = normalizeModerationAPIKeys(append(check.apiKeys(), newKeys...))
			}
			check.APIKey = ""
		}
		if input.APIKey != nil && strings.TrimSpace(*input.APIKey) != "" {
			check.APIKeys = normalizeModerationAPIKeys(append(check.APIKeys, *input.APIKey))
			check.APIKey = ""
		}
		if input.APIKeyUpdates != nil {
			if err := applyContentModerationAPIKeyMetadataUpdates(check, *input.APIKeyUpdates); err != nil {
				return err
			}
		}
		// 输入区显式填写的属性优先于旧 Key 草稿，便于用同一 Key 更新其权重和备注。
		applyContentModerationAPIKeyEntryMetadata(check, entries)
	}
	if err := s.validateConfig(ctx, check); err != nil {
		return err
	}
	p = &ContentModerationEngineConfig{BaseURL: check.BaseURL, Model: check.Model, ProxyID: check.ProxyID, APIKeys: check.APIKeys, TimeoutMS: check.TimeoutMS, RetryCount: check.RetryCount, Thresholds: check.Thresholds, APIKeyMetadata: check.APIKeyMetadata}
	if engine == ContentModerationEngineTypeSafe {
		cfg.TypeSafe = p
	} else {
		cfg.applyEngineProfile(p)
	}
	cfg.engineProfileApplied = false
	return nil
}
