      ...
      <template #table>
        ...
        >
          ...
          <template #cell-group="{ row }">
            <button
              v-if="row.smart_routing"
              type="button"
              data-test="smart-routing-group-summary"
              class="inline-flex max-w-[24rem] items-center gap-2 whitespace-nowrap rounded-lg px-2 py-1 text-left transition-colors hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 dark:hover:bg-dark-700"
              :title="smartRoutingSummaryTitle(row)"
              :aria-label="smartRoutingSummaryTitle(row)"
              aria-haspopup="dialog"
              @click="editKey(row)"
            >
              <!-- 候选顺序只用于选取首组；其余分组折叠为计数，避免撑高表格行。 -->
              <GroupBadge
                v-for="groupID in row.smart_routing_group_ids?.slice(0, 1)"
                :key="groupID"
                v-bind="smartRoutingBadgeProps(row, groupID)"
                class="min-w-0 max-w-[14rem] [&>span:not(.truncate)]:shrink-0"
              />
              <span
                class="inline-flex shrink-0 items-center gap-1 rounded-full bg-blue-50 px-2 py-1 text-xs font-medium text-blue-600 dark:bg-blue-900/20 dark:text-blue-400"
              >
                {{ t('keys.smartRouting.label') }}
                <span v-if="row.smart_routing_group_ids?.length > 1" class="tabular-nums">
                  +{{ row.smart_routing_group_ids.length - 1 }}
                </span>
              </span>
              <svg
                class="h-3.5 w-3.5 shrink-0 text-gray-400 opacity-60"
                aria-hidden="true"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
    ...
    >
      <form id="key-form" @submit.prevent="handleSubmit" class="key-form-controls min-w-0 max-w-full space-y-5">
        ...
        <div class="space-y-3">
          ...
          <label class="flex w-fit cursor-pointer items-center gap-2 text-sm text-gray-700 dark:text-dark-200">
            <input
              type="checkbox"
              :checked="formData.smart_routing"
              class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500 dark:border-dark-500"
              data-test="smart-routing-toggle"
              @change="onSmartRoutingModeChange(($event.target as HTMLInputElement).checked)"
            />
            {{ t('keys.smartRouting.label') }}
          </label>
          <p v-if="formData.smart_routing" class="text-sm leading-6 text-gray-500 dark:text-dark-400">
            {{ t('keys.smartRouting.hint') }}
          </p>
          <div v-if="formData.smart_routing" class="space-y-1.5">
            <label for="smart-routing-cooldown-seconds" class="input-label">
              {{ t('keys.smartRouting.cooldownLabel') }}
            </label>
            <input
              id="smart-routing-cooldown-seconds"
              v-model.number="formData.smart_routing_cooldown_seconds"
              type="number"
              min="0"
              max="3600"
              step="1"
              ...
              :class="{ 'border-red-500 dark:border-red-500': smartRoutingCooldownError }"
              :aria-invalid="Boolean(smartRoutingCooldownError)"
              aria-describedby="smart-routing-cooldown-hint smart-routing-cooldown-error"
              data-test="smart-routing-cooldown-seconds"
            />
            <p id="smart-routing-cooldown-hint" class="text-xs leading-5 text-gray-500 dark:text-dark-400">
              {{ t('keys.smartRouting.cooldownHint') }}
            </p>
            <p v-if="smartRoutingCooldownError" id="smart-routing-cooldown-error" class="text-xs text-red-500">
              {{ smartRoutingCooldownError }}
            </p>
          </div>
          <SmartRoutingGroupEditor
          ...
          />
          <Select
            v-else-if="!formData.is_composite"
        ...
        <div v-if="formData.is_composite" class="space-y-3" data-test="composite-group-editor">
          <div
        ...
        <!-- 分组停用时的请求级自动降级开关。 -->
        <div v-if="!formData.smart_routing" class="flex items-center justify-between">
...
import SmartRoutingGroupEditor from '@/components/keys/SmartRoutingGroupEditor.vue'
...
const formData = ref({
  ...
  smart_routing: false,
  smart_routing_group_ids: [] as number[],
  smart_routing_cooldown_seconds: 60 as number | '',
...
const smartRoutingGroupName = (key: ApiKey, groupID: number) =>
  key.smart_routing_groups?.find((group) => group.id === groupID)?.name ||
  groups.value.find((group) => group.id === groupID)?.name || `#${groupID}`

// 摘要复用普通分组的品牌和倍率展示，缺失详情时仍保留原候选 ID。
const smartRoutingBadgeProps = (key: ApiKey, groupID: number) => {
  const group = key.smart_routing_groups?.find((item) => item.id === groupID) ||
    groups.value.find((item) => item.id === groupID)
  return {
    name: group?.name || `#${groupID}`,
    platform: group?.platform,
    displayBrand: group?.display_brand,
    rateMultiplier: group?.rate_multiplier,
    userRateMultiplier: userGroupRates.value[groupID],
    peakRateEnabled: group?.peak_rate_enabled,
    peakStart: group?.peak_start,
    peakEnd: group?.peak_end,
    peakRateMultiplier: group?.peak_rate_multiplier
...
// 悬停和辅助技术可读取完整候选顺序，点击后仍在已有弹窗中查看和调整。
const smartRoutingSummaryTitle = (key: ApiKey) => [
  t('keys.smartRouting.editRoutes'),
  ...(key.smart_routing_group_ids || []).map((groupID, index) =>
    `${index + 1}. ${smartRoutingGroupName(key, groupID)}`
  )
].join('\n')

// 智能路由与显式前缀模式互斥，创建时可继承已选的普通分组。
const onSmartRoutingModeChange = (enabled: boolean) => {
  formData.value.smart_routing = enabled
  if (enabled) {
    formData.value.is_composite = false
    if (formData.value.smart_routing_group_ids.length === 0 && formData.value.group_id !== null) {
      formData.value.smart_routing_group_ids = [formData.value.group_id]
    }
  } else if (selectedKey.value?.smart_routing) {
    // 已保存的多组 Key 转回普通模式必须显式选择目标组。
    formData.value.group_id = null
  } else if (formData.value.group_id === null) {
    formData.value.group_id = formData.value.smart_routing_group_ids[0] ?? null
  }
}

// 空输入不可被转换成 0；0 只表示关闭当前密钥的分组冷却。
const smartRoutingCooldownError = computed(() => {
  if (!formData.value.smart_routing) return ''
  const seconds = formData.value.smart_routing_cooldown_seconds
  return typeof seconds !== 'number' || !Number.isFinite(seconds) ||
    !Number.isInteger(seconds) || seconds < 0 || seconds > 3600
    ? t('keys.smartRouting.cooldownInvalid')
    : ''
})

const smartRoutingFormError = computed(() => {
  if (!formData.value.smart_routing) return ''
  const ids = formData.value.smart_routing_group_ids
  if (ids.length === 0) return t('keys.smartRouting.groupRequired')
  if (ids.length > 10) return t('keys.smartRouting.limitReached')
  if (new Set(ids).size !== ids.length) return t('keys.composite.groupDuplicate')
  return smartRoutingCooldownError.value
})

// 切换复合模式时保留普通 Key 的原分组，前缀仍要求用户明确填写。
const onCompositeModeChange = (enabled: boolean) => {
  if (enabled) {
    formData.value.smart_routing = false
...
// 指定订阅切换后，各种 Key 模式都不能继续保留套餐未覆盖的分组。
const pruneFormGroupBindings = (allowedGroupIDs: Set<number>) => {
  ...
  formData.value.smart_routing_group_ids = formData.value.smart_routing_group_ids.filter((id) => allowedGroupIDs.has(id))
}
...
const editKey = (key: ApiKey) => {
  ...
  formData.value = {
    ...
    smart_routing: key.smart_routing ?? false,
    smart_routing_group_ids: [...(key.smart_routing_group_ids || [])],
    // 缺省沿用服务端默认值，同时保留已保存的 0 秒。
    smart_routing_cooldown_seconds: key.smart_routing_cooldown_seconds ?? 60,
...
const openGroupSelector = (key: ApiKey) => {
	if (key.is_composite || key.smart_routing || key.billing_mode === 'subscription') {
		editKey(key)
		return
	}
  ...
  try {
    if (showEditModal.value && selectedKey.value) {
      const updates: UpdateApiKeyRequest = {
        ...
        group_id: formData.value.is_composite || formData.value.smart_routing ? undefined : formData.value.group_id,
        is_composite: formData.value.is_composite,
        smart_routing: formData.value.smart_routing,
        smart_routing_group_ids: formData.value.smart_routing ? [...formData.value.smart_routing_group_ids] : undefined,
        smart_routing_cooldown_seconds: formData.value.smart_routing && typeof formData.value.smart_routing_cooldown_seconds === 'number'
          ? formData.value.smart_routing_cooldown_seconds : undefined,
        ...
        fallback_to_default_group_when_unavailable: !formData.value.smart_routing && formData.value.fallback_to_default_group_when_unavailable
      }
...
const handleSubmit = async () => {
	if (smartRoutingFormError.value) {
		appStore.showError(smartRoutingFormError.value)
		return
	}
  ...
  if (!formData.value.is_composite && !formData.value.smart_routing && formData.value.group_id === null) {
...
const resetQuotaUsed = async () => {
  ...
  try {
    ...
    // 只同步本次重置的密钥，以服务端返回值保留停用、过期等真实状态。
