...
// TestImageResultUploaderRejectsPrivateURL 防止受上游控制的下载地址探测本机及内网服务。
func TestImageResultUploaderRejectsPrivateURL(t *testing.T) {
	for _, address := range []string{"http://127.0.0.1:1/test.png", "http://169.254.169.254/latest/meta-data", "http://[::1]:1/test.png"} {
		uploader := NewImageResultUploader(&fakeImageStorage{}, "images/", 0, nil)
		body, err := json.Marshal(map[string]any{"data": []map[string]string{{"url": address}}})
		require.NoError(t, err)
		_, err = uploader.Rewrite(context.Background(), "imgtask_private", body)
		require.ErrorContains(t, err, "not public")
	}
}

// TestImageResultUploaderRejectsOversizedB64JSON 与 URL 和 data URL 统一限制解码后图片大小。
func TestImageResultUploaderRejectsOversizedB64JSON(t *testing.T) {
	storage := &fakeImageStorage{}
	uploader := NewImageResultUploader(storage, "images/", 3, nil)
	_, err := uploader.Rewrite(context.Background(), "imgtask_large", json.RawMessage(`{"data":[{"b64_json":"`+base64.StdEncoding.EncodeToString(pngBytes)+`"}]}`))
	require.ErrorContains(t, err, "exceeds")
	require.Empty(t, storage.saved)
}

// TestImageTaskUsesAcceptedStorageSnapshot 热关闭后已接收任务仍转存图片，不会把大结果写进 Redis。
func TestImageTaskUsesAcceptedStorageSnapshot(t *testing.T) {
	storage := &fakeImageStorage{}
	uploader := NewImageResultUploader(storage, "images/", 0, nil)
	enabled := true
	...
	require.NoError(t, err)
	snapshot, available := svc.ExecutionStorage()
	require.True(t, available)
	enabled = false
	result := json.RawMessage(`{"data":[{"b64_json":"` + base64.StdEncoding.EncodeToString(pngBytes) + `"}],"usage":{"input_tokens":12}}`)
	require.NoError(t, svc.CompleteWithUploader(context.Background(), task.ID, http.StatusOK, result, snapshot))
...
func TestImageResultUploaderRewritesURL(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		// 下载结果以实际位图格式为准，上游错误响应头不能影响对象的扩展名和 MIME。
