...
// Users Management
    users: {
      ...
      leaveEmptyToKeep: '未编辑则保留原密码，编辑为空则清除',
...
// Groups Management
    groups: {
      ...
      availabilityProbe: {
        ...
        protocolAutoHint: '自动沿用账号原有测试策略；自适应账号可能测试多个端点，任一端点失败都会使本轮探测失败。',
        protocolSelectedHint: '只测试所选上游协议，其他协议不可用不会影响本次结果；账号必须支持所选协议。',
        protocolOpenAIHint: 'OpenAI 的 Chat Completions 探测仅支持 API Key 账号。',
        protocolOpenCodeHint: 'Jev 模型使用 SystemOne 端点，请选择“自动”。',
        testNow: '立即测试',
        testing: '正在测试…',
        testNowHint: '保存当前探测配置并测试一次，结果计入渠道状态；其他分组修改请点击保存。本次不执行自动重试。',
        createBeforeTest: '创建并保存分组后，可在编辑窗口中立即测试。',
        testSuccess: '探测成功',
        testFailed: '探测失败',
        testLatency: '耗时 {ms} ms',
        testAccount: '账号 #{id}',
        testRequestFailed: '探测请求失败，请稍后重试',
        testBusy: '当前分组正在探测，请等待本轮结束后再试',
        interval: '探测间隔（分钟）',
        timeout: '超时时间（秒）',
        maxRetries: '最大重试次数',
