...
func TestAPIKeyUpdate_OnlyDeclaresRequestedColumns(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			req:  UpdateAPIKeyRequest{FastModePolicy: &fastModePolicy},
			want: APIKeyUpdateFields{FastModePolicy: true},
		},
		{
			name: "fork group fallback policy only",
			req:  UpdateAPIKeyRequest{FallbackToDefaultGroupWhenUnavailable: &fallbackToDefaultGroup},
			want: APIKeyUpdateFields{FallbackToDefaultGroupWhenUnavailable: true},
		},
	}
