...
import (
	...
	"github.com/TokenFlux/TokenRouter/internal/handler/quotaview"
...
// 管理员视角会懒归零过期窗口，并暴露 *_window_start 调试字段。
...
// 平台合法性由 service.IsAllowedQuotaPlatform 和 service.AllowedQuotaPlatforms 统一判断。

// UpdateUserPlatformQuotas 处理 PUT /admin/users/:id/platform-quotas。
// 全量替换该用户所有平台限额。
func (h *UserHandler) UpdateUserPlatformQuotas(c *gin.Context) {
	...
	// 在 UpsertForUser 之前抓取更新前快照，用于审计更新前后对比。
	// ListByUser 失败不阻断主操作，按尽力而为记录降级 warn。
...
// ResetUserPlatformQuotaWindow 处理 POST /admin/users/:id/platform-quotas/reset。
