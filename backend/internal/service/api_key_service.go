...
type APIKeyUpdateFields struct {
	...
	GroupID   bool
	ExpiresAt bool
	// CompositeConfiguration 覆盖两种多分组模式与有序关联，必须同事务更新。
	CompositeConfiguration bool
	// SmartRoutingCooldown 只更新冷却秒数，不重写有序候选关联。
	SmartRoutingCooldown bool
...
func validateCreateAPIKeyRequest(req CreateAPIKeyRequest) error {
	if req.SmartRoutingCooldownSeconds != nil {
		if err := ValidateSmartRoutingCooldownSeconds(*req.SmartRoutingCooldownSeconds); err != nil {
			return err
		}
	}
	limits := []struct {
...
func (s *APIKeyService) Create(ctx context.Context, userID int64, req CreateAPIKeyRequest) (*APIKey, error) {
	...
	if req.SmartRouting {
		if req.IsComposite || req.GroupID != nil || len(req.CompositeGroups) > 0 {
			return nil, ErrSmartRoutingConflict
		}
		compositeGroups, err = s.prepareSmartRoutingGroups(ctx, user, req.SmartRoutingGroupIDs)
		if err != nil {
			return nil, err
		}
	} else if len(req.SmartRoutingGroupIDs) > 0 {
		return nil, ErrSmartRoutingConflict
	} else if req.IsComposite {
		if req.GroupID != nil {
	...
	// 创建API Key记录
	smartRoutingCooldownSeconds := DefaultSmartRoutingCooldownSeconds
	if req.SmartRoutingCooldownSeconds != nil {
		smartRoutingCooldownSeconds = *req.SmartRoutingCooldownSeconds
	}
	apiKey := &APIKey{
		UserID:                                userID,
		TeamID:                                teamID,
		...
		SmartRouting:                          req.SmartRouting,
		SmartRoutingCooldownSeconds:           smartRoutingCooldownSeconds,
...
// SelectCompositeGroupForRequest 为复合或智能路由 Key 创建请求级分组视图，不修改缓存对象。
func (s *APIKeyService) SelectCompositeGroupForRequest(ctx context.Context, apiKey *APIKey, binding *APIKeyCompositeGroup) (*APIKey, error) {
	if apiKey == nil || (!apiKey.IsComposite && !apiKey.SmartRouting) || binding == nil || binding.Group == nil {
	...
	// 智能路由只能使用配置的候选，不允许停用分组回退改变已校验的模型目录。
	prepared := &selected
	if !apiKey.SmartRouting {
		prepared = s.applyDefaultGroupFallback(ctx, prepared)
	}
...
func (s *APIKeyService) Update(ctx context.Context, id int64, userID int64, req UpdateAPIKeyRequest) (*APIKey, error) {
	...
	originalSmartRouting := apiKey.SmartRouting
	...
	if req.SmartRoutingCooldownSeconds != nil {
		apiKey.SmartRoutingCooldownSeconds = *req.SmartRoutingCooldownSeconds
		fields.SmartRoutingCooldown = true
	}
	...
	targetSmartRouting := apiKey.SmartRouting
	if req.SmartRouting != nil {
		targetSmartRouting = *req.SmartRouting
	}
	if targetSmartRouting && (targetComposite || req.GroupID != nil || req.CompositeGroups != nil) {
		return nil, ErrSmartRoutingConflict
	}
	if !targetSmartRouting && req.SmartRoutingGroupIDs != nil && len(*req.SmartRoutingGroupIDs) > 0 {
		return nil, ErrSmartRoutingConflict
	}

	// 类型切换和复合映射更新必须在写库前一次性完成校验。
	if targetSmartRouting {
		if !apiKey.SmartRouting && req.SmartRoutingGroupIDs == nil {
			return nil, ErrSmartRoutingGroupsRequired
		}
		if req.SmartRoutingGroupIDs != nil {
			user, err := s.billingUserForAPIKey(ctx, apiKey)
			if err != nil {
				return nil, fmt.Errorf("get billing user: %w", err)
			}
			// 管理接口加载的 User 可能没有完整分组权限，绑定前读取当前权限。
			user, err = s.userRepo.GetByID(ctx, user.ID)
			if err != nil {
				return nil, fmt.Errorf("get user: %w", err)
			}
			bindings, err := s.prepareSmartRoutingGroups(ctx, user, *req.SmartRoutingGroupIDs)
			if err != nil {
				return nil, err
			}
		...
		apiKey.SmartRouting = true
		apiKey.IsComposite = false
		apiKey.GroupID = nil
		apiKey.Group = nil
	} else if targetComposite {
		if req.GroupID != nil {
			return nil, ErrCompositeKeyGroupConflict
		}
		...
		apiKey.SmartRouting = false
		apiKey.GroupID = nil
		apiKey.Group = nil
	} else if apiKey.IsComposite || apiKey.SmartRouting {
		if req.GroupID == nil || *req.GroupID <= 0 {
			if apiKey.SmartRouting {
				return nil, ErrSmartRoutingTargetRequired
			}
	...
	if apiKey.IsComposite != originalIsComposite || apiKey.SmartRouting != originalSmartRouting {
		fields.CompositeConfiguration = true
		fields.GroupID = true
	}

	if !targetComposite && !targetSmartRouting && req.GroupID != nil {
		// 验证分组权限
		billingUserID := userID
		if apiKey.TeamID != nil {
			if s.teamRepo == nil {
	...
	if !apiKey.IsComposite && !apiKey.SmartRouting && !s.canUserUseBoundGroup(ctx, apiKey) {
		return nil, ErrGroupDisabledForUser
	}
	// 切换套餐、普通分组或复合映射时，必须验证最终所有分组都在指定套餐范围内。
	if targetBillingMode == APIKeyBillingModeSubscription && (billingConfigurationRequested || req.GroupID != nil || req.CompositeGroups != nil || req.IsComposite != nil || req.SmartRouting != nil || req.SmartRoutingGroupIDs != nil) {
		if billingUser == nil {
			billingUser, err = s.billingUserForAPIKey(ctx, apiKey)
			if err != nil {
				return nil, fmt.Errorf("get billing user: %w", err)
