...
// GrokOAuthRefreshMutationRepository 使用上游尝试所用的精确凭据保护后台刷新失败更新。
// 该契约仅适用于 Grok，其它平台行为保持不变。
...
// newTokenRefreshRateGateWithInterval 是测试速率槽预约和取消的窄化时间注入点。
...
// processRefreshContext 执行一次有界且可从游标恢复的刷新周期。
...
// providerRateGate 返回后台周期与管理端对账共享的进程内限速器，避免并发入口或重试放大平台请求速率。
...
// providerConcurrencyGate 返回平台后台刷新与管理端对账共享的进程内信号量。
// 每次上游重试前后获取并释放，避免并行入口放大 ProviderConcurrency。
...
// clearAntigravityForceTokenRefresh 在刷新完成或确定不可恢复后清除一次性强制刷新标记。
