...
// Channel Management
    channels: {
      ...
      form: {
        ...
        reasoningEffortMultiplier: '{effort} reasoning multiplier',
        reasoningEffortMultipliersHint: 'Applies to token costs at the final forwarded effort. Empty values retain existing defaults (3x for Fable 5.1 Max, otherwise 1x). Image, per-second and search fees are unchanged.',
...
// Subscriptions
    subscriptions: {
      bulkSelected: '{count} subscriptions selected on this page',
      bulkExtend: 'Bulk extend',
      bulkReset: 'Bulk reset',
      bulkResume: 'Confirm previous batch operation',
      bulkRevoke: 'Batch Revoke',
      bulkRestore: 'Batch Restore',
      bulkSelectionHint: 'Only subscriptions selected on this page are included, up to 100 per batch. Extend and reset support active, pending and expired subscriptions; revoke and restore include eligible records only.',
      bulkConfirmTargets: 'Confirm the following {count} subscriptions:',
      bulkRevokeHint: 'Revoking stops access through these subscriptions and shifts later subscriptions for the same plan under the existing rules. The whole batch must succeed. Account balances are unchanged.',
      bulkRestoreHint: 'Restore the original validity and quota without extending expiry. Overlapping subscriptions for the same plan prevent the entire batch from being applied.',
      bulkRetryHint: 'This request is locked. Confirmation retries the same batch; closing and reopening keeps the pending operation.',
      batchAssign: {
        enable: 'Batch assignment',
        hint: 'Select up to 100 users. Subscriptions follow the existing plan rules for each user.',
        selected: '{count} users selected',
        removeUser: 'Remove {email}',
        result: '{success} succeeded, {failed} failed',
        retryHint: 'Successful users have been removed. Only the remaining failed users will be retried.',
        pendingHint: 'The previous assignment is unconfirmed and its parameters are locked. Retry confirmation or review assigned subscriptions to avoid duplicate grants.',
        blockedHint: 'This assignment is unconfirmed. Review subscriptions already granted before any new assignment. You can confirm the original request.',
        confirmResult: 'Confirm original request',
        retry: 'Retry confirmation'
      },
      bulkExtendDays: 'Days to add',
      bulkExtendHint: 'Add days to the current expiry and shift later subscriptions of the same plan. Only the last expired subscription in a plan can be extended, starting from now. The entire batch must succeed.',
      bulkResetWindows: 'Usage windows to reset',
      bulkResetHint: 'Clear usage in the selected windows without changing expiry or quota limits. The entire batch must succeed.',
      bulkSelectionLimit: 'Select between 1 and 100 subscriptions',
      bulkInvalidStatus: 'Only active, pending, and expired subscriptions support bulk actions. Deselect suspended or revoked records.',
      bulkInvalidDays: 'Days to add must be an integer between 1 and 36500',
      bulkSelectWindow: 'Select at least one usage window to reset',
      bulkSuccess: 'Successfully updated {count} subscriptions',
      bulkFailed: 'The batch result could not be confirmed. Confirm again to retry the same batch.',
      bulkExpiredHasSuccessor: 'Expired subscription #{id} has later subscriptions in the same plan. Deselect it and extend the latest subscription instead. No changes were made to this batch.',
