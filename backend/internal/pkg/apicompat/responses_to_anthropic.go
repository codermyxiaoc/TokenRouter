...
// responsesTextPart 同时以输出项和内容索引标识正文片段。
...
type ResponsesEventToAnthropicState struct {
	...
	// textByPart 记录已交付正文；关闭内容块后仍保留，用于去重和补充尾文。
	textByPart map[responsesTextPart]*strings.Builder
	// textDelivered 表示已经生成过客户端正文事件。
	textDelivered bool
...
// @project-doc docs/interfaces/openai_upstream.md#responses_bridge_compatibility
func ResponsesEventToAnthropicEvents(
	...
	case "response.reasoning_summary_text.done":
		// 保持 thinking 块开启直到 response.output_item.done。Grok/Codex 会在完成的
		// reasoning 项上附加 encrypted_content；提前关闭会丢失 signature_delta，
		// 导致多轮缓存失效。
		return nil
...
// resToAnthEmitText 按需开启文本块，并登记已交付片段以供 done 对账。
...
// resToAnthRecoverText 只补已输出前缀之后的尾文；不匹配时不重发正文。
...
func resToAnthHandleFuncArgsDone(evt *ResponsesStreamEvent, state *ResponsesEventToAnthropicState) []AnthropicStreamEvent {
	// 上游可能在 completed 关闭 block 后补发或重复发送 done，已关闭时保持幂等。
...
func resToAnthHandleOutputItemDone(evt *ResponsesStreamEvent, state *ResponsesEventToAnthropicState) []AnthropicStreamEvent {
	...
	// 在 reasoning 项完成时捕获 encrypted_content，它通常只会出现在此事件中。
...
// resToAnthRecoverTerminalText 恢复仅在终态 output 中提供的正文。
// 只有此前未交付正文时才恢复，因为终态数组索引与增量索引不保证一致。
...
func closeCurrentBlock(state *ResponsesEventToAnthropicState) []AnthropicStreamEvent {
	...
	idx := state.ContentBlockIndex
	var events []AnthropicStreamEvent
	// 在停止事件前发出 signature_delta，使 Claude 客户端为下一轮保留加密推理；
	// Grok 多轮缓存依赖该数据。
