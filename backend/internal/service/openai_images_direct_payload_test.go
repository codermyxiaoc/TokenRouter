...
func TestCodexDirectImagesPricingAndUsage(t *testing.T) {
	...
	require.Equal(t, 2e-6, catalog["gpt-image-2"].CacheReadInputImageTokenCost)
	...
	for _, stream := range []bool{false, true} {
		t.Run(fmt.Sprint(stream), func(t *testing.T) {
			...
			upstream := &directImagesHTTPUpstreamStub{do: func(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
				...
				return &http.Response{StatusCode: 400, Header: http.Header{}, Body: io.NopCloser(strings.NewReader(`{"error":{"type":"invalid_request_error","message":"The 'gpt-5.4-mini' model is not supported when using Codex with a ChatGPT account."}}`))}, nil
			}}
