...
HostService API v2 的列表响应同时保留旧 `account_ids` 与结构化 `accounts`，旧插件仍可只读取 ID。结构化记录包含账号类型、状态、可调度标记和脱敏目录元数据；宿主再次按原作用域过滤，不因 v2 扩大到 API Key、其他平台或影子账号。元数据不包含 Credentials、Extra、Proxy 对象及分组循环引用，避免泄露密钥、代理密码或扩展私有字段；需要出站凭据仍须调用受范围检查的独立解析接口。
