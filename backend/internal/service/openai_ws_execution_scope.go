...
const (
	...
	// 键与当前主道相同；明确声明但认不出的取值各自成道，宁可只在同类间
	// 互相替换，也不让未知副请求打断用户轮次。
...
func resolveOpenAIWSClientThreadID(c *gin.Context, body []byte) string {
	body = openAIWSExecutionPayload(body)
...
func openAIWSExecutionScopeSeed(apiKeyID int64, identity, value, lane string) string {
	// 客户端可自定义标识，长度前缀防止 thread/session 内的分隔符伪造另一个请求类型。
	return fmt.Sprintf("openai_ws_exec:%d|%s=%d:%s|lane=%d:%s", apiKeyID, identity, len(value), value, len(lane), lane)
}
...
// 不是可靠身份，不参与抢占。
// @project-doc docs/interfaces/openai_upstream.md#openai_execution_scope
...
// openAIWSExecutionPayload 兼容 response.create 的嵌套 response 信封，只切片原始报文。
func openAIWSExecutionPayload(body []byte) []byte {
	if gjson.GetBytes(body, "type").String() != "response.create" {
		return body
	}
	view := parseRawJSONView(body).Get("response")
	if view.Type == gjson.JSON && view.Index > 0 && view.Index+len(view.Raw) <= len(body) {
		return body[view.Index : view.Index+len(view.Raw)]
	}
	return body
}

const openAIWSExecutionScopeContextKey = "openai_ws_original_execution_scope"

// rememberOpenAIWSExecutionScope 在账号 namespace/指纹收敛前保存身份，同次 failover 不重新派生。
// 空值也必须保存，防止后续账号注入的默认 thread_id 被误认为客户端身份。
func rememberOpenAIWSExecutionScope(c *gin.Context, body []byte) string {
	if c == nil {
		return ""
	}
	...
	c.Set(openAIWSExecutionScopeContextKey, scope)
	return scope
}
