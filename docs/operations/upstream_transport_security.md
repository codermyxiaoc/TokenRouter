...
- `proxy`：沿显式 fallback proxy 链寻找启用且未过期的目标。
...
备用代理是单向多对一关系：多个主代理可以共用备用出口，也可以配置 A→B→C；创建、更新或清空 A 的备用配置不得反写 B 的备用字段。Ent 使用显式 `fallback_sources` 反向边表达该关系，数据库沿用已有 nullable 外键和普通索引，不增加迁移，也不自动改写历史链路。旧版本曾形成的循环继续按循环保护拒绝回退，应由管理员修正配置。

到期扫描的快照不是最终决策：落库前原子核对代理仍启用、仍过期，且到期时间、回退模式和目标与扫描快照一致。管理员续期或修改回退策略后，旧扫描不能覆盖新设置。多级回退继续移动当前绑定账号，同时保留最初的 `proxy_fallback_origin_id`；恢复原代理的归属不会被中间跳点覆盖。调度快照与 fork 的 Ollama 用量快照仍通过原失效路径更新。
...
响应体生命周期使用独立的上游子 context。提前关闭响应体时先取消阻塞读，再串行关闭解压器和连接，避免压缩流读取与 Close 相互等待；完整读取后仍允许 keep-alive 复用。该取消只终止当前 transport 请求，不能取消父级计费 context 或已经取得的用量收尾。
