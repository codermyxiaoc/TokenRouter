...
运行时系统日志设置中的 `request_retention_days` 独立控制普通 `usage_logs` 自动留存：`0` 为永久，正数范围为 1–3650 天；旧客户端未提交该字段时保留当前值，旧配置缺失时沿用部署端 usage retention，原聚合关闭的部署默认继续永久保留。独立定时触发器每分钟检查、实际清理至少间隔 6 小时，因此关闭预聚合不妨碍显式设置的留存生效。配置读取失败或非法时跳过删除，不能回退成更短期限。

自动清理不会修改完整错误详情的保存/清理链路。计费幂等去重保留窗口至少覆盖普通用量留存，永久用量也跳过去重删除；已有更长去重配置仍优先。分区表先回收完整过期分区，再按时间条件清理边界分区，批次用 `(tableoid, ctid)` 定位，避免不同分区相同 `ctid` 误删。余额、套餐和结算事实不受这一设置更改。
...
- 余额流水、订单、订阅和结算事实不随 usage/Ops retention 自动删除；计费去重记录由独立配置约束且不得早于关联用量留存期。
...
<a id="backup_monthly_archives"></a>
### 月度归档与恢复状态

定时备份可选择每月日期、月末和独立归档数量；当月不存在的日期归并到月末。按 cron 时区和备份开始日期计算到期归档日，当月首次成功覆盖未处理日期的定时备份进入归档池并持久推进水位；手动备份不自动占用归档日期，删除记录不会回退水位。有限归档与普通保留数量分别清理，永久归档不被后续改小的配置降级。手动删除归档必须显式提交 `delete_archived=true`。

备份记录和归档水位在同一跨实例写锁内保存；优先使用 Redis，同一部署不能在 Redis 错误时静默换数据库锁。恢复中的记录受删除与清理保护；恢复操作以开始时间区分，迟到的失败回调不能覆盖新操作或复活已删除记录。周期恢复扫描对超过 35 分钟的挂起任务标记失败，旧记录首次发现没有恢复时间戳时先补时间并给予完整宽限。保留原本地、S3、分卷校验和数据库维护锁，不改变备份内容默认排除规则。
