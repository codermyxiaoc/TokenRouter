...
// geminiSignalUnexpectedEOFReader 模拟已收到错误帧后发生的上游连接截断。
type geminiSignalUnexpectedEOFReader struct{ io.Reader }

func (r geminiSignalUnexpectedEOFReader) Read(p []byte) (int, error) {
...
func TestGeminiForwardNative_AggregateReadFailureKeepsObservedSignal(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, tc := range []struct {
		name, body string
		wantSignal bool
	}{
		{"observed 503", "data: {\"response\":{\"error\":{\"code\":503,\"status\":\"UNAVAILABLE\",\"message\":\"temporary unavailable\"}}}\n\n", true},
		{"empty interrupted read", "", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			upstream := &geminiCompatHTTPUpstreamStub{response: &http.Response{
				StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"text/event-stream"}, "X-Request-Id": []string{"aggregate-failure"}},
				Body: io.NopCloser(geminiSignalUnexpectedEOFReader{strings.NewReader(tc.body)}),
			}}
			svc := &GeminiMessagesCompatService{cfg: &config.Config{}, tokenProvider: &GeminiTokenProvider{}, httpUpstream: upstream}
			account := &Account{ID: 704, Platform: PlatformGemini, Type: AccountTypeOAuth, Credentials: map[string]any{"access_token": "test-token", "project_id": "test-project"}}
			c, rec := newGeminiNativeTestContext(t)
			result, err := svc.ForwardNative(context.Background(), c, account, "gemini-3.7-flash", "generateContent", false, geminiSignalTestRequest())
			require.Error(t, err)
			require.Nil(t, result)
			require.Equal(t, http.StatusBadGateway, rec.Code)
			require.Contains(t, rec.Body.String(), "Failed to read upstream stream")
			marks := GetOpsStreamErrors(c)
			if !tc.wantSignal {
			...
			require.Equal(t, http.StatusServiceUnavailable, marks[0].IntendedStatus)
			require.Equal(t, "UNAVAILABLE", marks[0].Code)
			require.True(t, marks[0].NonStream)
			events := upstreamErrorEventsFromContext(t, c)
			require.Len(t, events, 1)
			require.Equal(t, int64(704), events[0].AccountID)
			require.Equal(t, "aggregate-failure", events[0].UpstreamRequestID)
