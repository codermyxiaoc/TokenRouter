...
func APIKeyFromService(k *service.APIKey) *APIKey {
	...
	out := &APIKey{
		...
		SmartRouting:                          k.SmartRouting,
		SmartRoutingCooldownSeconds:           k.SmartRoutingCooldownSeconds,
		SmartRoutingGroupIDs:                  k.SmartRoutingGroupIDs(),
	...
	for _, binding := range k.CompositeGroups {
		// 内部候选前缀不属于智能路由的公共协议，避免误导客户端加前缀。
		if k.SmartRouting {
			if group := GroupFromServiceShallow(binding.Group); group != nil {
				out.SmartRoutingGroups = append(out.SmartRoutingGroups, group)
			}
			continue
		}
...
func userSubscriptionFromServiceBase(sub *service.UserSubscription) UserSubscription {
	return UserSubscription{
		...
		DailyResetCount:    sub.DailyResetCount,
		WeeklyResetCount:   sub.WeeklyResetCount,
		MonthlyResetCount:  sub.MonthlyResetCount,
