...
- 智能路由 Key 按一次重定向后的目标模型匹配候选账号目录，并选中首个有当前可调度账号且未冷却的分组。支持安全重放的请求在组内最终上游 429/5xx 后依序换组，每轮重新鉴权；已输出业务内容、已有结算或本地策略拒绝不重放，详见[智能路由 API Key](../domains/smart_routing_api_keys.md#group_failover)。
