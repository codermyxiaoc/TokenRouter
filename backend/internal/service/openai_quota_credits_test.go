...
func TestQueryUsageCodexCredits(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			svc := NewOpenAIQuotaService(stubQuotaAdminService{repo: repo}, &stubQuotaHTTPUpstream{redirectTarget: target}, NewOpenAITokenProvider(repo, tokens, nil), nil, nil)
