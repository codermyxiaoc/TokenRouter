...
// Channel Management
    channels: {
      ...
      form: {
        ...
        reasoningEffortMultiplier: '{effort} 推理倍率',
        reasoningEffortMultipliersHint: '按最终发送的推理档位计算 Token 费用；留空沿用原规则（Fable 5.1 的 Max 默认 3 倍，其余默认 1 倍）。不影响按张、按秒或搜索附加费用。',
...
// Subscriptions Management
    subscriptions: {
      bulkSelected: '当前页已选 {count} 个订阅',
      bulkExtend: '批量延长',
      bulkReset: '批量重置',
      bulkResume: '继续确认上次批量操作',
      bulkRevoke: '批量撤销',
      bulkRestore: '批量恢复',
      bulkSelectionHint: '仅操作当前页所选订阅，每批最多 100 个。延长和重置支持生效中、待生效及已过期订阅；撤销和恢复仅处理可操作状态的记录。',
      bulkConfirmTargets: '请确认以下 {count} 个订阅：',
      bulkRevokeHint: '撤销后停止使用该订阅，同套餐后续订阅按现有规则前移。本批全部成功后生效，不影响账户余额。',
      bulkRestoreHint: '恢复原订阅的有效期与额度，不自动延期。若与同套餐其他订阅时间重叠，整批不作修改。',
      bulkRetryHint: '本次请求参数已锁定。重试确认会使用同一批次；关闭后再次打开仍保留待确认操作。',
      batchAssign: {
        enable: '批量分配',
        hint: '最多选择 100 位用户，按当前套餐规则为每位用户分配订阅。',
        selected: '已选择 {count} 位用户',
        removeUser: '移除 {email}',
        result: '成功 {success} 位，失败 {failed} 位',
        retryHint: '成功用户已移除，再次提交只处理剩余失败用户。',
        pendingHint: '上次分配结果尚未确认，参数已锁定。请重试确认或核对用户已获订阅，避免重复发放。',
        blockedHint: '本次分配结果尚未确认，请先核对已发放订阅，勿重新分配；可使用原请求确认结果。',
        confirmResult: '确认原请求结果',
        retry: '重试确认'
      },
      bulkExtendDays: '增加天数',
      bulkExtendHint: '在原到期时间上增加天数，同套餐后续订阅顺延。仅允许延长同套餐最后一份已过期订阅，并从当前时间起算。整批成功后生效。',
      bulkResetWindows: '选择要重置的用量',
      bulkResetHint: '将所选周期的已用额度清零，不改变订阅到期时间和额度上限。整批成功后生效。',
      bulkSelectionLimit: '每次请选择 1 至 100 个订阅',
      bulkInvalidStatus: '批量操作仅支持生效中、待生效和已过期的订阅，请取消选择已暂停或已撤销的记录',
      bulkInvalidDays: '增加天数必须为 1 至 36500 的整数',
      bulkSelectWindow: '请至少选择一个需要重置的周期',
      bulkSuccess: '已成功更新 {count} 个订阅',
      bulkFailed: '批量操作未确认成功，请点击确认重试同一批次',
      bulkExpiredHasSuccessor: '订阅 #{id} 已过期且存在同套餐后续记录，请取消选择该历史订阅，改为延长最后一份订阅。本批未作修改。',
      title: '订阅管理',
