...
func TestBuildUsageBillingCommandSimpleModeOnlyChargesAPIKeyWindows(t *testing.T) {
	...
	require.Zero(t, cmd.BillableAmountUSD)
	require.Zero(t, cmd.BaseAmountUSD)
	...
	for _, subscription := range []bool{false, true} {
		t.Run(strconv.FormatBool(subscription), func(t *testing.T) {
			...
			p := &usageBillingParams{
				...
				Account: &Account{ID: 9}, Subscription: func() *UserSubscription {
					if subscription {
			...
			finalizeUsageBilling(p, &billingDeps{
