...
describe('UserPlatformQuotaCell', () => {
  ...
  it('多平台按 anthropic→openai→gemini→antigravity→qoder→grok 顺序，仅展示有限额的', () => {
    const w = mount(UserPlatformQuotaCell, {
      props: {
        quotas: [
          item({ platform: 'gemini', monthly_limit_usd: 50 }),
          item({ platform: 'grok', monthly_limit_usd: 60 }),
          item({ platform: 'qoder', monthly_limit_usd: 55 }),
          item({ platform: 'antigravity', monthly_limit_usd: 52 }),
    ...
    expect(text.indexOf('antigravity')).toBeLessThan(text.indexOf('qoder'))
    expect(text.indexOf('qoder')).toBeLessThan(text.indexOf('grok'))
