...
// 上游明确以 finish_reason 完成空内容时，即使省略 [DONE] 也须返回完整 Anthropic 流。
func TestForwardAsAnthropic_ForceChatCompletionsEmptyCompletionStillFramesMessage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	upstream := &httpUpstreamRecorder{resp: &http.Response{
		...
		Body:       io.NopCloser(strings.NewReader("data: {\"id\":\"chatcmpl_empty\",\"choices\":[{\"index\":0,\"delta\":{},\"finish_reason\":\"stop\"}]}\n\n")),
	}}
	svc := &OpenAIGatewayService{
		cfg:          rawChatCompletionsTestConfig(),
		httpUpstream: upstream,
