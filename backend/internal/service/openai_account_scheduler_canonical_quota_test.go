...
func TestOpenAISchedulerCanonicalResetScoresMatchSnapshot(t *testing.T) {
	...
	accounts := []*Account{
		{ID: 1, Platform: PlatformOpenAI, SessionWindowEnd: &sessionEnd, Extra: map[string]any{"codex_5h_reset_at": now.Add(10 * time.Minute).Format(time.RFC3339)}},
		{ID: 2, Platform: PlatformOpenAI, SessionWindowEnd: &sessionEnd, Extra: map[string]any{"codex_5h_reset_at": now.Add(4 * time.Hour).Format(time.RFC3339)}},
	}
	weights := GatewayAdvancedSchedulerScoreWeightsView{Reset: 1}
	candidates, _, ranges := scoreAdvancedSchedulerCandidatesWithRanges(accounts, nil, nil, weights, advancedSchedulerSelectionInput{}, now)
	require.InDelta(t, 1, candidates[0].score-candidates[1].score, 1e-9)
	snapshots := buildAdvancedAccountSchedulerScoreSnapshot(accounts, nil, nil, nil, weights, false, nil)
	for i := range candidates {
		candidate := &candidates[i]
		require.InDelta(t, candidate.score, snapshots[candidate.account.ID].BaseScore, 1e-9)
		metric := diagnosticResetMetric(candidate, ranges, weights, now)
		require.True(t, metric.Available)
		require.Equal(t, "account.extra.codex_5h_reset_at", metric.Source)
		require.Equal(t, candidate.factors.Reset, metric.NormalizedValue)
	}
	// 其它平台继续使用会话列，不能误读携带的 Codex 字段。
	accounts[0].Platform = PlatformGemini
	end, ok := advancedSchedulingResetWindowEnd(accounts[0], now)
	require.True(t, ok)
