...
官方 OpenCode 请求默认使用 `opencode/1.0.0` User-Agent，官方 CommandCode 默认使用项目的 Codex 客户端身份；管理员配置的 TLS 路由 UA、账号 UA 继续优先，账号显式 Header Override 最后应用。Cloudflare 1010 仍记录请求失败，但不会累加账号级 403 惩罚。
