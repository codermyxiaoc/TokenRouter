...
// QueryDisabledPublicUsers chains the current query on the "disabled_public_users" edge.
func (_q *GroupQuery) QueryDisabledPublicUsers() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *GroupQuery) QueryUserDisabledPublicGroups() *UserDisabledPublicGroupQuery {
...
// WithDisabledPublicUsers tells the query-builder to eager-load the nodes that are connected to
// the "disabled_public_users" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithDisabledPublicUsers(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withDisabledPublicUsers = query
	return _q
}
...
func (_q *GroupQuery) WithUserDisabledPublicGroups(opts ...func(*UserDisabledPublicGroupQuery)) *GroupQuery {
	query := (&UserDisabledPublicGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withDisabledPublicUsers; query != nil {
		if err := _q.loadDisabledPublicUsers(ctx, query, nodes,
			func(n *Group) { n.Edges.DisabledPublicUsers = []*User{} },
			func(n *Group, e *User) { n.Edges.DisabledPublicUsers = append(n.Edges.DisabledPublicUsers, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadDisabledPublicUsers(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int64]*Group)
	nids := make(map[int64]map[*Group]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.DisabledPublicUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.DisabledPublicUsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.DisabledPublicUsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.DisabledPublicUsersPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "disabled_public_users" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *GroupQuery) loadUserDisabledPublicGroups(ctx context.Context, query *UserDisabledPublicGroupQuery, nodes []*Group, init func(*Group), assign func(*Group, *UserDisabledPublicGroup)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userdisabledpublicgroup.FieldGroupID)
	}
