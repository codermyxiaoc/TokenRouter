...
普通注册表单在浏览器校验密码与确认密码一致；确认值不加入注册请求或临时持久化。服务端仍使用既有密码与注册校验，OAuth 独立入口不增加该表单要求。
...
用户和管理员 JWT 的用户读取都区分身份失效与基础设施故障：明确的用户不存在返回 `401 USER_NOT_FOUND`，数据库超时或连接故障返回 `500 INTERNAL_ERROR`。两类情况都拒绝当前私有请求，读取失败不得放行，也不得把临时故障错误标记为用户被删除。
...
刷新遇到网络故障、`429` 或 `5xx` 时，HTTP 客户端返回 `TOKEN_REFRESH_UNAVAILABLE` 并保留浏览器凭据，当前请求失败，服务恢复后可再次刷新。明确的无效、过期或撤销等认证失败仍按原规则清理失效会话；登录和 One Tap 等认证入口继续自行处理其错误。用户资料刷新以及新凭据首次加载资料也保留暂时不可用时的凭据，并在落盘或清理前检查请求所属会话，防止旧资料响应覆盖或删除后来建立的会话。保留凭据不保证服务端已撤销的 refresh token 可以再次使用，也不构成绕过鉴权。
