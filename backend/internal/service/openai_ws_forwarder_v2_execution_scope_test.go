...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	captureEvents := [][]byte{completed("resp_a"), completed("resp_b"), completed("resp_c")}
	handshake := http.Header{}
	handshake.Set(openAIWSTurnStateHeader, "turn-state-from-upstream")
	pool := newOpenAIWSConnPool(cfg)
	pool.setClientDialerForTest(&openAIWSExecutionScopeFreshDialer{events: captureEvents, handshake: handshake})
	defer pool.Close()
	...
	injectedScope, _ := deriveOpenAISessionHashes(openAIWSExecutionScopeSeed(apiKeyID, "thread", injected.threadID, ""))
...
// fork 的 full 指纹会把稳定身份纳入连接硬兼容键；每次拨号必须返回真正独立的连接。
// 不能重复返回已因身份变化被淘汰的 mock，否则会把正确隔离误报成转发失败。
type openAIWSExecutionScopeFreshDialer struct {
	events    [][]byte
	handshake http.Header
}

func (d *openAIWSExecutionScopeFreshDialer) Dial(_ context.Context, _ string, _ http.Header, _ string, _ *tlsfingerprint.Profile) (openAIWSClientConn, int, http.Header, error) {
	return &openAIWSCaptureConn{events: append([][]byte(nil), d.events...)}, 0, d.handshake.Clone(), nil
}
