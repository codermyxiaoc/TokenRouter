...
// grokCompactSummaryPrompt 对应 grok-build 的 build_compaction_prompt(None, false)。
...
func buildGrokCompactRequestBody(body []byte) ([]byte, error) {
	...
	if err := decodeGrokCompactJSON(body, &payload); err != nil {
...
func convertOpenAICompactInputsForGrok(body []byte) ([]byte, error) {
	...
	for _, raw := range items {
		...
		if !ok || !isResponsesCompactionItemType(grokCompactStringValue(item["type"])) {
...
func convertGrokResponseToOpenAICompact(body []byte) ([]byte, error) {
	...
	if err := decodeGrokCompactJSON(body, &response); err != nil {
...
func decodeGrokCompactJSON(body []byte, value any) error {
	if !json.Valid(body) {
