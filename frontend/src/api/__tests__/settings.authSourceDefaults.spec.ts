...
describe("admin settings auth source defaults helpers", () => {
  ...
  it("preserves MiniMax platform quotas through normalization and serialization", () => {
    ...
    const state = buildAuthSourceDefaultsState({ auth_source_default_email_platform_quotas: quotas });
    const payload: UpdateSettingsRequest = {};
    appendAuthSourceDefaultsToUpdateRequest(payload, state);
    expect(payload.auth_source_default_email_platform_quotas?.minimax).toEqual(quotas.minimax);
  });
