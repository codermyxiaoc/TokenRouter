...
智能路由 Key 选组后携带固定请求范围，包括 `simple` 模式在内的数据库候选、快照返回和最终账号归属检查都必须限制在该分组；简易模式仍可复用原平台缓存桶，但不能把桶内其它分组账号直接交给智能请求。
...
基础调度器保留原有的优先级、最近使用、负载、粘性和等待路径，不因高级调度器的存在改变排序或失败语义。基础模式同样优先尝试 `previous_response_id` 绑定，重新检查当前分组、请求模型准入、最终账号模型、传输/端点能力与 privacy；不合格或已移组时释放占用再回退原选择流程。决策标签按实际命中的上一响应或 session 填写，不能把最终普通选号误记为粘性命中。读取分组策略使用轻量投影，账号缓存投影保留 RPM 字段，避免缓存路径绕开数据库中的限流配置。

高级调度器只在各平台先完成现有硬过滤后接管候选排序：
