...
type UpdateAPIKeyRequest struct {
	...
	IsComposite *bool  `json:"is_composite"`
	// nil 保持当前配置，候选数组使用完整替换语义。
	SmartRouting         *bool    `json:"smart_routing"`
	SmartRoutingGroupIDs *[]int64 `json:"smart_routing_group_ids"`
	// nil 保留当前配置，显式 0 关闭冷却。
	SmartRoutingCooldownSeconds *int `json:"smart_routing_cooldown_seconds"`
...
func validateAPIKeyCreateRequest(req CreateAPIKeyRequest) error {
	if req.SmartRoutingCooldownSeconds != nil {
		if err := service.ValidateSmartRoutingCooldownSeconds(*req.SmartRoutingCooldownSeconds); err != nil {
			return err
		}
	}
...
func (h *APIKeyHandler) Create(c *gin.Context) {
	...
	svcReq := service.CreateAPIKeyRequest{
		...
		SmartRoutingGroupIDs:                  req.SmartRoutingGroupIDs,
