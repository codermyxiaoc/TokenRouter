...
func createAPIKeyRecord(ctx context.Context, client *dbent.Client, key *service.APIKey) (*dbent.APIKey, error) {
	builder := client.APIKey.Create().
		...
		SetSmartRouting(key.SmartRouting).
		SetSmartRoutingCooldownSeconds(key.SmartRoutingCooldownSeconds).
...
func (r *apiKeyRepository) GetByKeyForAuth(ctx context.Context, key string) (*service.APIKey, error) {
	m, err := r.activeQuery().
		...
		Select(
			...
			apikey.FieldSmartRouting,
			apikey.FieldSmartRoutingCooldownSeconds,
			apikey.FieldName,
			apikey.FieldStatus,
...
func (r *apiKeyRepository) Update(ctx context.Context, key *service.APIKey, fields service.APIKeyUpdateFields) error {
	...
	if fields.CompositeConfiguration {
		...
		builder.SetSmartRouting(key.SmartRouting)
	}
	if fields.SmartRoutingCooldown {
		builder.SetSmartRoutingCooldownSeconds(key.SmartRoutingCooldownSeconds)
	}
...
func (r *apiKeyRepository) apiKeyListByUserIDQuery(userID int64, filters service.APIKeyListFilters) *dbent.APIKeyQuery {
	...
	if filters.GroupID != nil {
		if *filters.GroupID == 0 {
			q = q.Where(apikey.GroupIDIsNil(), apikey.IsCompositeEQ(false), apikey.SmartRoutingEQ(false))
		} else {
			q = q.Where(apikey.Or(
...
func apiKeyEntityToService(m *dbent.APIKey) *service.APIKey {
	...
	out := &service.APIKey{
		...
		SmartRouting:                          m.SmartRouting,
		SmartRoutingCooldownSeconds:           m.SmartRoutingCooldownSeconds,
		Quota:                                 m.Quota,
