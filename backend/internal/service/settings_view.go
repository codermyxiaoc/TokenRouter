...
type SystemSettings struct {
	...
	ClaudeCodeClientVersionSynced    string
...
//   - "flex"：低优先级模式
//   - 省略：normal 默认；管理员可用专用 missing 条件显式匹配
//
...
const (
	...
	OpenAIFastTierMissing   = "missing"   // 仅匹配未指定 service_tier 的请求
...
type OpenAIFastPolicyRule struct {
	ServiceTier          string   `json:"service_tier"`                     // 已识别档位、all 或专用 missing 条件
