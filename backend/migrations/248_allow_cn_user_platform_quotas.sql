...
-- 注册时 GetDefaultPlatformQuotas 会为全部 9 平台预填充默认配额行，但旧 CHECK 未包含
-- 三个国产平台。BulkInsertInitial 是单条多行 INSERT，任一违约行会中止整条
-- 语句 → 注册路径 fail-open 吞错 → 新用户拿到零条配额记录（含原有平台，缺失配额
-- 行 = 无限额）。与 157 头注释记载的 grok 同型事故一致。
--
-- 修复：把约束与代码平台列表（PlatformKimi/PlatformZhipu/PlatformDeepseek）对齐。
-- DROP ... IF EXISTS 保证可重入；新约束是旧约束的超集，不回填任何存量用户。
