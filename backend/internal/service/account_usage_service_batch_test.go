...
func (r *usageBatchLogRepoStub) GetUsageRanking(context.Context, time.Time, time.Time, int, UsageRankingSortBy) (*usagestats.UsageRankingResponse, error) {
	return nil, nil
}
...
// 模拟 ClearError 的持久化效果，同时检查写入和仓储返回对象的状态变化。
