...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	if apiKey.Group != nil && !apiKey.SmartRouting {
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if apiKey != nil && (apiKey.IsComposite || apiKey.SmartRouting) {
...
// compositeRequestableModels 按候选顺序聚合模型，只有复合 Key 添加展示前缀。
...
func (h *GatewayHandler) compositeRequestableModels(c *gin.Context, apiKey *service.APIKey, requiredPlatform string) []string {
	...
	for _, binding := range apiKey.CompositeGroups {
		...
		if apiKey.SmartRouting {
			// 智能 Key 的列表和选组共用有限目录，避免价卡或任意透传生成虚假候选。
			var err error
			available, err = service.NewSmartRoutingService(h.gatewayService, h.openAIGatewayService).ResolveSmartRoutingModels(ctx, group)
			if err != nil {
				continue
			}
		} else {
		...
		for _, model := range available {
			prefixed := model
			// 智能路由公开原始模型并按候选顺序去重，复合 Key 继续保留前缀。
