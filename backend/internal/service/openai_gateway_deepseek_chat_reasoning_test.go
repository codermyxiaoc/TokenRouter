...
func (c *reasoningHitCache) SetReasoningContent(_ context.Context, itemID, content string, _ time.Duration) error {
	if c.values == nil {
		c.values = make(map[string]string)
	}
	if _, exists := c.values[itemID]; !exists {
		c.values[itemID] = content
	}
	return nil
}
