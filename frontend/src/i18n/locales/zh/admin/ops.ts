...
// Ops Monitoring
    ops: {
      ...
      diagnosis: {
        ...
        diskCritical: '磁盘空间使用率严重过高 ({usage}%)',
        diskCriticalImpact: '可能导致日志、缓存或数据库写入失败',
        diskCriticalAction: '清理无用文件、检查日志轮转，并扩容磁盘空间',
        diskHigh: '磁盘空间使用率偏高 ({usage}%)',
        diskHighImpact: '剩余空间不足，需要关注',
        diskHighAction: '检查大文件与日志增长趋势，准备扩容或清理',
