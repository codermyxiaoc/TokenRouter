...
// NewProvider 创建基于 macOS ChatGPT App DeviceCheck 模块的证明提供器。
...
// resolveRuntime 仅接受官方 ChatGPT App 内随包分发的运行时和原生模块。
...
// Generate 在短时子进程中生成并校验 DeviceCheck attestation JSON。
...
// readSignals 读取 ChatGPT App 证明所需的有限设备信号并约束字段长度。
