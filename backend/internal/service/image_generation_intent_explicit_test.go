...
func TestIsExplicitImageGenerationIntentMap_AfterRequestMutation(t *testing.T) {
	...
	assert.False(t, IsExplicitImageGenerationIntentMap("/v1/responses", "gpt-5.5", passiveNamespace))
	...
	assert.True(t, IsExplicitImageGenerationIntentMap("/v1/responses", "gpt-5.5", nativeTool))

	explicitNamespaceChoice := map[string]any{
		"model":       "gpt-5.5",
		"tool_choice": map[string]any{"type": "namespace", "name": "image_gen"},
	}
	assert.True(t, IsExplicitImageGenerationIntentMap("/v1/responses", "gpt-5.5", explicitNamespaceChoice))
}
