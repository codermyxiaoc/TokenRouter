<template>
  <AppLayout full-viewport>
    ...
    <div class="ticket-workspace mx-auto flex min-h-0 max-w-5xl flex-col gap-3 overflow-hidden">
      <div class="flex shrink-0 items-center justify-between gap-3"><RouterLink :to="base" class="text-sm text-primary-600 hover:underline dark:text-primary-400">← {{ t('tickets.back') }}</RouterLink><button type="button" class="btn btn-secondary btn-sm" :disabled="loading || busy" @click="load">{{ t('common.refresh') }}</button></div>
      ...
      <p v-if="error" role="alert" class="max-h-24 shrink-0 overflow-y-auto rounded-xl bg-red-50 p-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-300">{{ error }}</p>
      ...
      <template v-if="ticket">
        <section class="ticket-info card min-w-0 shrink-0 p-3 sm:p-4">
          <div class="ticket-title-row flex min-w-0 items-start justify-between gap-2"><div class="ticket-title-block min-w-0 flex-1"><p class="text-xs text-gray-500">#{{ ticket.id }} · {{ t(`tickets.types.${ticket.type}`) }}</p><h1 class="mt-1 truncate text-lg font-semibold text-gray-900 dark:text-white" :title="ticket.title">{{ ticket.title }}</h1></div><div class="flex shrink-0 gap-2"><TicketBadge kind="priority" :value="ticket.priority" /><TicketBadge kind="status" :value="ticket.status" :closed-by-role="ticket.closed_by_role" /></div></div>
          <dl class="ticket-metadata mt-3 grid min-w-0 grid-cols-2 gap-2 text-xs sm:grid-cols-3"><div v-if="admin" class="min-w-0"><dt class="text-gray-500">{{ t('tickets.user') }}</dt><dd class="mt-1 truncate" :title="`${ticket.user_name} · ${ticket.user_email}`">{{ ticket.user_name }} · {{ ticket.user_email }}</dd></div><div><dt class="text-gray-500">{{ t('tickets.createdAt') }}</dt><dd class="mt-1" :title="formatDate(ticket.created_at)">{{ formatDate(ticket.created_at) }}</dd></div><div v-if="ticket.order_id" class="min-w-0"><dt class="text-gray-500">{{ t('tickets.order') }}</dt><dd class="ticket-order-summary mt-1 flex min-w-0 flex-wrap gap-x-2" :title="`${ticket.order?.out_trade_no || `#${ticket.order_id}`}${ticket.order ? ` · ${ticket.order.currency || 'CNY'} ${ticket.order.pay_amount.toFixed(2)}` : ''}`"><span class="truncate" :title="ticket.order?.out_trade_no || `#${ticket.order_id}`">{{ ticket.order?.out_trade_no || `#${ticket.order_id}` }}</span><span v-if="ticket.order" class="shrink-0">{{ ticket.order.currency || 'CNY' }} {{ ticket.order.pay_amount.toFixed(2) }}</span></dd></div></dl>
          <div v-if="isOpen" class="ticket-actions mt-3 flex flex-wrap items-center gap-3 border-t border-gray-100 pt-3 dark:border-dark-700">
            <template v-if="admin"><div class="flex items-center gap-2"><label for="detail-priority" class="text-sm text-gray-500">{{ t('tickets.priority') }}</label><Select id="detail-priority" class="w-28" :model-value="ticket.priority" :options="priorityOptions" :disabled="busy" @update:model-value="changePriority" /></div></template>
            <div class="flex flex-1 justify-end gap-2"><button type="button" class="btn btn-secondary btn-sm" :disabled="busy || loading" @click="closeAction = 'cancel'">{{ t('tickets.cancel') }}</button><button type="button" class="btn btn-primary btn-sm" :disabled="busy || loading" @click="closeAction = 'complete'">{{ t('tickets.complete') }}</button></div>
          </div>
        </section>
        <section class="flex min-h-0 flex-1 flex-col gap-2" :aria-label="t('tickets.conversation')">
          ...
          <div ref="conversationElement" data-testid="ticket-messages" role="region" tabindex="0" aria-labelledby="ticket-conversation-heading" class="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain pr-1" @scroll="trackConversationScroll">
          <article v-for="message in ticket.messages || []" :key="message.id" class="rounded-2xl border p-5" :class="message.is_staff ? 'border-primary-100 bg-primary-50/50 dark:border-primary-900/50 dark:bg-primary-900/10' : 'border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900'">
            <div class="flex flex-wrap items-center justify-between gap-2 text-xs"><div class="flex items-center gap-2"><span class="font-semibold text-gray-900 dark:text-white">{{ message.sender_name || (message.is_staff ? t('tickets.staff') : ticket.user_name || t('tickets.user')) }}</span><span v-if="message.is_staff" class="rounded bg-primary-100 px-2 py-0.5 text-primary-700 dark:bg-primary-900/30 dark:text-primary-300">{{ t('tickets.staff') }}</span></div><time :datetime="message.created_at" class="text-gray-500">{{ formatDate(message.created_at) }}</time></div>
            <!-- 对话内容作为纯文本展示，避免上传者把消息转为可执行 HTML。 -->
            <p class="mt-4 whitespace-pre-wrap break-words text-sm leading-7 text-gray-800 dark:text-gray-200">{{ message.content }}</p>
            <ul v-if="message.attachments?.length" class="mt-4 flex flex-wrap gap-2"><li v-for="attachment in message.attachments" :key="attachment.id" class="max-w-full"><button type="button" class="flex max-w-full items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs text-primary-600 hover:bg-gray-50 dark:border-dark-600 dark:bg-dark-800 dark:text-primary-400" :aria-label="t('tickets.preview.openNamed', { name: attachment.filename })" @click="previewAttachment = attachment"><Icon name="document" size="sm" /><span class="min-w-0 max-w-64 truncate" :title="attachment.filename">{{ attachment.filename }}</span><span class="shrink-0 text-gray-500">{{ formatAttachmentSize(attachment.size) }}</span></button></li></ul>
          </article>
          </div>
        </section>
        <form v-if="isOpen && settings" data-testid="ticket-composer" class="ticket-composer card max-h-[40dvh] shrink-0 space-y-2 overflow-y-auto p-3 sm:p-4" @submit.prevent="reply">
...
/* 软键盘或横屏压缩高度时优先保留输入区，辅助信息不挤占剩余对话空间。 */
@media (max-height: 550px) {
  .ticket-workspace { gap: 0.375rem; }
  .ticket-info { padding: 0.5rem; }
  ...
  .ticket-composer { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 0.25rem 0.5rem; padding: 0.5rem; }
