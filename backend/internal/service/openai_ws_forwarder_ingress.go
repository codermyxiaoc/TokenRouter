...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	// 仅命中已启用插件能力的账号改用 HTTP Bridge，保留其他 WS 调度策略。
	forceHTTPBridge := account.Platform == PlatformGrok || (s.pluginManager != nil && s.pluginManager.ShouldRouteOpenAIOAuth(account))
	...
	refreshIngressRouteState := func(payload openAIWSClientPayload) {
		...
		// 账号粘性仍由 handler 管理；这里只隔离线程/请求种类的执行状态。
		scope, _ := resolveOpenAIWSExecutionScope(c, payload.rawForHash, getAPIKeyIDFromContext(c))
		...
		if useHTTPBridge {
			// 独立 HTTP 桥接不得继承 native WS 的 turn-state 或连接绑定。
			return
		}
	...
	acquireTurnLease = func(turn int, preferred string, forcePreferredConn bool, forceNewConn bool) (*openAIWSConnLease, error) {
		...
		// 独占模式及上游读写失败后的安全重试均新建连接，避免重复选择陈旧空闲连接。
