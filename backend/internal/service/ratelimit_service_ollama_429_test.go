...
func (p *ollama429SchedulerStub) ScheduleOllamaCloudUsageRateLimitProbe(accountID int64, cb OllamaCloudUsageRateLimitProbeCallback) bool {
	...
	p.callbacks[accountID] = cb // 与协调器一致，保留最新回调
	return true
}
...
func (r *ollama429Repo) BeginOllamaCloudRateLimit(_ context.Context, id int64, resetAt *time.Time) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	a := r.accounts[id]
	if a == nil {
	...
	if resetAt != nil && (cur == nil || resetAt.After(*cur)) {
		a.RateLimitResetAt = cloneTimePtr(resetAt)
		r.ifLaterWrites = append(r.ifLaterWrites, *resetAt)
	}
	advanceOllamaTestRateLimitGeneration(a)
	r.bump(a)
	return nil
}

// 与数据库一致：即使恢复时间相同也必须递增代次，避免平台时钟精度使测试偶发通过。
func advanceOllamaTestRateLimitGeneration(a *Account) {
	now := time.Now().UTC().Truncate(time.Microsecond)
	if a.RateLimitedAt != nil && !now.After(*a.RateLimitedAt) {
		now = a.RateLimitedAt.Add(time.Microsecond)
	}
	a.RateLimitedAt = &now
}
...
func TestHandle429_OllamaEarlyBranchAcrossPlatforms(t *testing.T) {
	// 当前 fork 的 Ollama 识别仅覆盖 OpenAI/Anthropic 兼容账号，不扩大到国产供应商。
...
func TestHandle429_OllamaFallsBackToSecondsCooldown(t *testing.T) {
	...
	before := time.Now()
	svc.handle429(context.Background(), acct, http.Header{}, nil) // 未提供重试时间
	after := time.Now()
...
func TestHandle429_OllamaScheduleRejectedStillCooled(t *testing.T) {
	...
	repo := newOllama429Repo(acct)
	scheduler := newOllama429SchedulerStub(false) // 队列已满或服务停止
	svc, _ := ollama429Fixture(t, repo, scheduler)
...
func TestOllamaProbeCallback_ValidUpdatesDBAndMemory(t *testing.T) {
	...
	svc.handle429(context.Background(), acct, http.Header{}, nil) // 排队探测并立即冷却五秒
	require.Equal(t, 1, scheduler.count())
...
func TestOllamaProbeCallback_StaleLongDoesNotOverrideNewShort(t *testing.T) {
	...
	svc.handle429(context.Background(), acct, http.Header{}, nil) // 捕获五秒冷却的代次
	require.Equal(t, 1, scheduler.count())
...
// 两次真实 429 即使仍在同一个长冷却内，也不能复用首个请求捕获的代次。
func TestOllamaProbeCallback_New429WithSameFloorInvalidatesOldCallback(t *testing.T) {
	acct := ollama429Account(423, PlatformOpenAI)
	longFloor := time.Now().Add(time.Hour)
	acct.RateLimitResetAt = &longFloor
	repo := newOllama429Repo(acct)
	scheduler := newOllama429SchedulerStub(true)
	svc, _ := ollama429Fixture(t, repo, scheduler)
	svc.handle429(context.Background(), acct, http.Header{}, nil)
	firstLimited := cloneTimePtr(acct.RateLimitedAt)
	oldCallback := scheduler.callbacks[acct.ID]
	svc.handle429(context.Background(), acct, http.Header{}, nil)
	require.True(t, acct.RateLimitedAt.After(*firstLimited))
	require.Equal(t, longFloor, *acct.RateLimitResetAt)
	oldCallback(acct.ID, time.Now().Add(7*24*time.Hour))
	require.Zero(t, repo.casUpdated)
	require.Equal(t, longFloor, *repo.currentReset(acct.ID))
}
...
func TestOllamaProbeCallback_KeySwapSkips(t *testing.T) {
	...
	svc.handle429(context.Background(), acct, http.Header{}, nil) // 捕获原凭据配置指纹
...
// 配置变化不能被探测写快照引起的正常版本更新掩盖。
func TestOllamaProbeCallback_AccountConfigurationChangesSkip(t *testing.T) {
	for name, mutate := range map[string]func(*Account){
		"proxy":       func(a *Account) { id := int64(2); a.ProxyID = &id },
		"session":     func(a *Account) { a.Extra = map[string]any{OllamaCloudUsageSessionExtraKey: "new-session"} },
		"base_url":    func(a *Account) { a.Credentials["base_url"] = "https://ollama.com/v1" },
		"credentials": func(a *Account) { a.Credentials["api_protocol"] = "responses" },
		"priority":    func(a *Account) { a.Priority++ },
		"name":        func(a *Account) { a.Name = "edited" },
	} {
		t.Run(name, func(t *testing.T) {
			acct := ollama429Account(804, PlatformOpenAI)
			repo := newOllama429Repo(acct)
			scheduler := newOllama429SchedulerStub(true)
			svc, _ := ollama429Fixture(t, repo, scheduler)
			svc.handle429(context.Background(), acct, http.Header{}, nil)
			repo.mutate(acct.ID, mutate)
			scheduler.fire(acct.ID, time.Now().Add(time.Hour))
			require.Zero(t, repo.casUpdated)
		})
	}
}
...
func TestOllamaProbeCallback_SnapshotPersistBumpStillApplies(t *testing.T) {
	...
	repo.bumpVersion(acct.ID) // 模拟快照写入引起的行版本变化
...
func (r *ollama429LinkRepo) UpdateOllamaCloudUsageSnapshot(ctx context.Context, expected *Account, snapshot *OllamaCloudUsageSnapshot) error {
	...
	r.mu.Lock()
	defer r.mu.Unlock()
	if a, ok := r.accountServiceTestRepo.accounts[expected.ID]; ok {
		r.bump(a)
	}
	return nil
}

func (r *ollama429LinkRepo) BeginOllamaCloudRateLimit(_ context.Context, id int64, resetAt *time.Time) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	a := r.accountServiceTestRepo.accounts[id]
	if a == nil {
		return nil
	}
	cur := a.RateLimitResetAt
...
func TestOllama429RealProbeLinkage_SnapshotPersistThenWriteBack(t *testing.T) {
	...
	repo := &ollama429LinkRepo{ollamaUsageTestRepo: &ollamaUsageTestRepo{
		accountServiceTestRepo: &accountServiceTestRepo{
			accounts: map[int64]*Account{account.ID: account},
		},
	}}
	upstream := &ollamaUsageHTTPStub{body: body}

	usageSvc := NewOllamaCloudUsageService(repo, upstream, NewSettingService(&settingServiceTestRepo{}, nil), ollamaUsageTestEncryptor{}, true)
