...
func (s *BillingCacheService) checkRPM(ctx context.Context, user *User, group *Group) error {
	...
	if group != nil {
		...
		if override != nil {
			...
			if *override > 0 {
				count, incErr := smartRoutingRPMCount(ctx, user.ID, group.ID, func() (int, error) {
					return s.userRPMCache.IncrementUserGroupRPM(ctx, user.ID, group.ID)
				})
				if incErr != nil {
					logger.LegacyPrintf(
						"service.billing_cache",
			...
			count, err := smartRoutingRPMCount(ctx, user.ID, group.ID, func() (int, error) {
				return s.userRPMCache.IncrementUserGroupRPM(ctx, user.ID, group.ID)
			})
			if err != nil {
				logger.LegacyPrintf(
	...
	// ── 第二层：用户级全局硬上限（始终生效） ──
	if user.RPMLimit > 0 {
		count, err := smartRoutingRPMCount(ctx, user.ID, 0, func() (int, error) {
			return s.userRPMCache.IncrementUserRPM(ctx, user.ID)
		})
		if err != nil {
			logger.LegacyPrintf(
				"service.billing_cache",
