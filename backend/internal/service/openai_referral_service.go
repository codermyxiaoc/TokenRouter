...
// referralCapacity 同时满足桌面端的发送名额和奖励名额限制。
// 桌面端另有每批最多五封的限制；此接口每次只发送一封。
...
func (s *OpenAIQuotaService) referralCall(ctx context.Context, id int64, program string) (OpenAIReferralCall, error) {
	...
	accountCtx, err := s.prepareAccount(ctx, id)
	if err != nil {
		return OpenAIReferralCall{}, err
	}
	// 不复用额度请求的 Agent task 自动重放：邀请发送没有幂等键，异常必须保留未知结果。
	do := func(req *http.Request) (*http.Response, error) {
		if _, err := s.applyHeaders(req, accountCtx); err != nil {
			return nil, err
		}
		*req = *req.WithContext(WithHTTPUpstreamRedirectsDisabled(req.Context()))
		return s.httpUpstream.DoWithTLS(req, accountCtx.proxyURL, accountCtx.account.ID, accountCtx.account.Concurrency, accountCtx.tlsProfile)
	}
	return OpenAIReferralCall{ProxyURL: accountCtx.proxyURL, ProgramID: program, Do: do}, nil
}
