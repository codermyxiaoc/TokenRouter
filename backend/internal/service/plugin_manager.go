...
type PluginManager struct {
	...
	// bindingStateLoaded 由 operationMu 保护，区分首次未知与已确认无插件绑定。
	bindingStateLoaded bool
...
// @project-doc docs/interfaces/local_plugins.md#plugin_lifecycle
func (m *PluginManager) Start(ctx context.Context) error {
	...
	// 未安装插件时不创建运行目录，目录故障只由实际启用插件的初始化路径处理。
...
func (m *PluginManager) Get(ctx context.Context, id int64) (*PluginInstallation, error) {
	installation, err := m.getInstalledPlugin(ctx, id)
	if err != nil {
		return nil, err
	}
	installation.Compatibility = EvaluatePluginCompatibility(installation.Manifest, m.hostInfo)
...
// 管理端读取将缺失记录归一化为 404；仓储层仍保留 sql.ErrNoRows，避免改变安装查重语义。
func (m *PluginManager) getInstalledPlugin(ctx context.Context, id int64) (*PluginInstallation, error) {
	installation, err := m.repo.GetByID(ctx, id)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, infraerrors.NotFound("PLUGIN_NOT_FOUND", "插件不存在").WithCause(err)
	}
...
func (m *PluginManager) reconcileOnce(ctx context.Context) error {
	...
	if err != nil {
		// 已知绑定只阻断原有灰度范围；已确认无绑定的旧业务保持原链路。
		// 首次启动未读取成功时不能假设插件未启用，避免绕过已持久化绑定。
		if current := m.route.Load(); current != nil {
			m.publishUnavailableRoute(current.pluginID, current.rolloutPercent, "插件启用状态暂时无法读取")
		} else if !m.bindingStateLoaded {
			m.publishUnavailableRoute(0, 100, "插件启用状态暂时无法读取")
		}
		return fmt.Errorf("读取插件启用状态: %w", err)
	}
	m.bindingStateLoaded = true
...
// Status 只读取运行中的插件 Health，不启动临时进程、不应用配置、不访问上游。
// 管理端可轮询该通道而无需二次验证，访问仍受管理员认证约束。
// @project-doc docs/interfaces/local_plugins.md#plugin_status_ui
func (m *PluginManager) Status(ctx context.Context, id int64) (*pluginv1.HealthResponse, error) {
	if _, err := m.getInstalledPlugin(ctx, id); err != nil {
		return nil, err
	}
	m.mu.Lock()
	runtime := m.runtimes[id]
...
func (m *PluginManager) ShouldRouteOpenAIOAuth(account *Account) bool {
	if m == nil || account == nil || account.Platform != PlatformOpenAI || account.Type != AccountTypeOAuth || account.Status != StatusActive || account.IsShadow() {
		return false
	}
	route := m.route.Load()
...
// @project-doc docs/interfaces/local_plugins.md#plugin_host_scope
...
// pluginDeclaresOpenAIOAuthCapability 检查已验证清单是否声明 OpenAI OAuth 出站能力。
