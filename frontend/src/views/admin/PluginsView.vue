...
// 配置页切换或 iframe 导航后，旧请求不能向新页面投递凭据或配置。
...
async function openConfiguration(plugin: PluginInstallation): Promise<void> {
  ...
  try {
    ...
    if (generation === bridgeGeneration) uiSession.value = session;
  } catch (error: unknown) {
...
function clearPendingBridgeRequests(): void {
  for (const pending of pendingBridgeRequests.values()) window.clearTimeout(pending.timeout);
  pendingBridgeRequests.clear();
}

function handlePluginFrameLoad(): void {
  // 插件主动导航也会触发 load，避免延迟返回的配置进入新文档。
...
function registerBridgeRequest(requestID: string): PendingBridgeRequest {
  const timeout = window.setTimeout(() => {
    pendingBridgeRequests.delete(requestID);
  }, 30_000);
  const pending = { timeout, generation: bridgeGeneration };
  pendingBridgeRequests.set(requestID, pending);
  return pending;
}
...
function postBridgeResult(
  ...
  if (!requestID || !pending || pendingBridgeRequests.get(requestID) !== pending || pending.generation !== bridgeGeneration) return;
  ...
  pluginFrame.value.contentWindow.postMessage(
    ...
    // 沙箱 iframe 的来源为 null；以窗口、会话令牌、请求实例与导航代次共同隔离。
    "*",
  );
}
...
async function handleBridgeMessage(event: MessageEvent): Promise<void> {
  ...
  let pending: PendingBridgeRequest | undefined;
  if (expectsResponse) {
    if (!requestID || requestID.length > 200 || pendingBridgeRequests.has(requestID) || pendingBridgeRequests.size >= 128) return;
    pending = registerBridgeRequest(requestID);
  }
  const pluginID = configPlugin.value.id;
  const generation = bridgeGeneration;
  const ensureCurrentSession = () => {
    // 二次验证等待期间关闭或切换配置页时，不向新插件提交旧配置。
    if (generation !== bridgeGeneration || configPlugin.value?.id !== pluginID) {
  ...
  try {
    switch (message.type) {
      ...
      case "config.load": {
        ...
        postBridgeResult(message, { ok: true, config }, pending);
        break;
      }
      case "config.save": {
        if (
        ...
        const config = await pluginStepUp.run(() => {
          ...
          return adminAPI.plugins.saveConfig(
            pluginID,
      ...
      case "config.test": {
        ...
        postBridgeResult(message, { ok: result.success, result }, pending);
        // 成功结果交由插件页面展示，避免轮询测试时反复弹出成功通知。
        if (!result.success)
          appStore.showError(result.message || t("common.error"));
        break;
      }
      case "plugin.status": {
        // 状态通道只读取健康快照，无需二次验证；展示和轮询由插件界面负责。
        const result = await adminAPI.plugins.status(pluginID);
        postBridgeResult(message, { ok: true, result }, pending);
        break;
      }
      case "ui.resize": {
