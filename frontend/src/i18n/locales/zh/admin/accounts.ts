    ...
    accounts: {
      ...
      cnProviders: {
        accountMode: {
          ...
          minimaxPaygDesc: '按 Token 从账户余额计费，当前不支持余额查询；Coding Plan 支持用量查询。',
          coding: 'Coding Plan',
      ...
      openaiQuotaReset: {
        ...
        resetSuccess: '重置成功，已恢复 {windows} 个窗口',
        resetCacheRefreshFailed: '重置已成功，但剩余次数刷新失败。请重新查询次数，不要重复重置。',
        resetAccountRecoveryFailed: '重置已成功，但本地账号限流状态恢复失败。请刷新账号并检查状态，不要重复重置。',
        resetAccountRefreshFailed: '重置已成功，但账号状态刷新失败。请刷新账号列表，不要重复重置。',
        resetUnknown: '无法确认重置结果，次数可能已消耗。请先查询额度和剩余次数，不要直接重复重置。',
        confirmTitle: '确认重置额度',
      ...
      openai: {
        ...
        wsMode: 'WS mode',
        wsModeDesc:
          'off 关闭当前 OpenAI 账号的 WS 能力；其他选项启用 WS，仍受网关全局开关和账号类型开关约束。',
        wsModeRoutingDesc:
          '全局 gateway.openai_ws.mode_router_v2_enabled=true 时，按所选模式分流（含 http_bridge）；为 false（默认）时，已启用的账号沿用旧版上下文池链路。',
        wsModeOff: '关闭（off）',
        ...
        wsModeConcurrencyHint:
          '所选模式生效后：上下文池在账号内复用上游 WS 连接。启用动态容量时按账号并发 × 系数计算，并受每账号连接上限约束；实际请求并发不变，账号并发需大于 0。',
        wsModePassthroughHint:
          '所选模式生效后：客户端与上游通过 WS 逐连接转发，不复用上下文池；启用自动桥接时，大体积首帧可能改走 HTTP 桥接。',
        wsModeHttpBridgeHint:
          '所选模式生效后：接收客户端 WS 请求，通过 HTTP/SSE 请求上游，再将结果转回客户端 WS；不使用上游 WS 连接池。',
        ...
        workloadCapabilitiesDesc: '控制该账号参与文本生成、Embeddings 或 Seedance 视频任务调度；视频任务需要显式开启。',
        workloadTextGeneration: '文本生成',
        workloadEmbeddings: 'Embeddings',
        workloadSeedance: 'Seedance（火山方舟视频）',
      ...
      grok: {
        ...
        mediaTooLarge: '文件过大（图片最多 4MB，音频最多 6MB）。',
        mediaInvalidType: '不支持该文件格式，请选择图片或音频文件。',
      ...
      testStreamInterrupted: '测试连接已中断，未收到完成结果。',
      testEndpoint: '测试端点',
      testEndpointHint: '仅影响本次测试，不修改账号协议配置；上游是否支持以实际返回为准。',
      testEndpoints: {
        auto: '自动（使用账号配置）',
