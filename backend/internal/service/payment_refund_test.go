...
func TestExecuteRefundUsesActualAvailableBalanceDeduction(t *testing.T) {
	...
	repo := &mockUserRepo{deductBalanceResultFn: func(_ context.Context, id int64, amount float64) (float64, error) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			svc := &PaymentService{
				...
				userRepo: &mockUserRepo{deductBalanceResultFn: func(ctx context.Context, id int64, amount float64) (float64, error) {
...
func TestFinalizePendingRefundSuccessRejectsStaleCallerBeforeSecondDeduction(t *testing.T) {
	...
	pendingDetail := (&PaymentService{entClient: client}).latestRefundPendingDetail(ctx, order.ID)
