...
func (s *GeminiMessagesCompatService) tryStickySessionHit(
	...
	// 智能路由不能复用历史 simple 共享池留下的跨组粘性绑定。
	if isSmartRoutingScoped(ctx) && (groupID == nil || !openAIStickyAccountMatchesGroup(account, groupID)) {
		return nil
	}
...
func (s *GeminiMessagesCompatService) ForwardNative(ctx context.Context, c *gin.Context, account *Account, originalModel string, action string, stream bool, body []byte) (*ForwardResult, error) {
	...
	if stream {
		...
		if useUpstreamStream {
			...
			collected, usageObj, stats, err := collectGeminiSSEObserved(resp.Body, isOAuth, func(raw []byte) {
				if sig, ok := detectGeminiResponseSignal(raw); ok && sig.Kind > best.Kind {
					best = sig
				}
			})
			if err != nil {
				// 聚合读失败不能抹掉此前已观测的上游信号；未读到信号时不制造空响应错误。
...
// geminiSSECollectStats 保存是否收到业务事件和有界的非 SSE 兜底正文。
type geminiSSECollectStats struct {
	dataEvents int
	fallback   *geminiSSEFallbackBody
}

// collectGeminiSSEObserved 在保留本地 reasoning/图片/工具 part 聚合的同时观测错误信号。
func collectGeminiSSEObserved(body io.Reader, isOAuth bool, observe func([]byte)) (map[string]any, *ClaudeUsage, geminiSSECollectStats, error) {
	...
	for {
		...
		if len(line) > 0 {
			...
			if !strings.HasPrefix(trimmed, "data:") {
				...
				case "", "[DONE]":
					if payload == "[DONE]" {
						return mergeCollectedGeminiParts(pickGeminiCollectResult(last, lastWithParts), collectedParts), usage, stats, nil
					}
				default:
...
func (s *GeminiMessagesCompatService) handleNativeStreamingResponse(c *gin.Context, resp *http.Response, startTime time.Time, isOAuth bool, account *Account, upstreamRequestID string) (*geminiNativeStreamResult, error) {
	...
	fallback := &geminiSSEFallbackBody{}
	readComplete := false
	// 中途读失败仍保留已观测错误；取消导致的空读不能被误报为空响应。
	defer func() {
		if readComplete || best.Kind != geminiSignalNone {
