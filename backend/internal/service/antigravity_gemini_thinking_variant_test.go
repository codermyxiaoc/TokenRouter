...
func TestResolveGeminiThinkingVariant(t *testing.T) {
	...
	tests := []struct {
		...
		// 通配符的目标恰好等于裸名时，不能把它误当成自动补齐的自映射，
		// 否则候选变体的精确映射会覆盖管理员原本选择的上游模型。
		{"wildcard identity target wins over exact variant", map[string]string{
			"gemini-*":              "gemini-3.8-flash",
			"gemini-3.8-flash-high": "gemini-3.8-flash-tiered",
		}, "gemini-3.8-flash", budget("-1"), "", false},
		{"more specific wildcard identity target wins over variant", map[string]string{
