...
func (s *SchedulerSnapshotService) ListSchedulableAccounts(ctx context.Context, groupID *int64, platform string, hasForcePlatform bool) ([]Account, bool, error) {
	...
	if s.cache != nil {
		...
		if err != nil {
			...
			// simple 模式保留共享 bucket；智能路由只读取其中属于最终分组的成员。
			return filterSmartRoutingAccounts(ctx, derefAccounts(cached), groupID), useMixed, nil
		}
	...
	return filterSmartRoutingAccounts(ctx, accounts, groupID), useMixed, nil
}
...
func (s *SchedulerSnapshotService) handleBulkAccountEvent(ctx context.Context, payload map[string]any, seen map[batchSeenKey]struct{}) error {
	...
	for _, account := range accounts {
		...
		case PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformQoder, PlatformGrok:
...
func schedulerSnapshotPlatforms() []string {
	return []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformQoder, PlatformGrok}
}
