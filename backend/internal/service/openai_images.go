...
func (s *OpenAIGatewayService) ForwardImages(
	...
	case AccountTypeOAuth, AccountTypeSetupToken:
		// 原生端点与兼容回退共用本次已匹配的 UA/TLS 路由，避免中途重新取默认身份。
...
func (s *OpenAIGatewayService) handleOpenAIImagesStreamingResponse(
	...
	c *gin.Context,
	startTime time.Time,
	directRequest ...*OpenAIImagesRequest,
) (OpenAIUsage, int, []string, *int, error) {
	var direct *OpenAIImagesRequest
	if len(directRequest) > 0 {
		direct = directRequest[0]
	}
	// 普通 API Key 透传保持原行为；仅原生 OAuth 把中途断流标成不可安全重放。
	readError := func(err error) error {
		if direct != nil && shouldClassifyOpenAIUpstreamStreamReadError(err) {
	...
	processSSEData := func(dataBytes []byte) {
		...
		if firstTokenMs == nil && strings.TrimSpace(gjson.GetBytes(dataBytes, "b64_json").String()) != "" {
	...
	processLine := func(line []byte) {
		...
		if firstTokenMs == nil && direct == nil {
			ms := int(time.Since(startTime).Milliseconds())
			firstTokenMs = &ms
		}
	...
	if streamInterval <= 0 && keepaliveInterval <= 0 {
		...
		for {
			...
			if err != nil {
				flushSSEEvent()
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, readError(err)
			}
		}
		flushSSEEvent()
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				flushSSEEvent()
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, readError(ev.err)
			}
			processLine(ev.line)
		case <-intervalCh:
			...
			if direct != nil {
				return usage, imageCounter.Count(), imageCounter.Sizes(), firstTokenMs, newOpenAIUpstreamStreamReadError(ErrOpenAIUpstreamStreamTruncated)
			}
			if clientDisconnected {
