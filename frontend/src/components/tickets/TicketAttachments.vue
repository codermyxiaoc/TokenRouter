<template>
  ...
  <div class="min-w-0 max-w-full space-y-2">
    ...
    <input v-if="settings.max_attachments > 0" :id="id" type="file" multiple :accept="attachmentAccept" :disabled="disabled" class="block w-full min-w-0 max-w-full text-sm text-gray-500 file:mr-3 file:rounded-lg file:border-0 file:bg-primary-50 file:px-4 file:py-2 file:text-primary-700 dark:file:bg-dark-700 dark:file:text-gray-200" @change="addFiles" />
    <ul v-if="modelValue.length" class="space-y-1">
      <li v-for="(file, index) in modelValue" :key="index" class="flex items-center justify-between gap-2 rounded-lg bg-gray-50 px-3 py-2 text-sm dark:bg-dark-800">
        ...
        <button type="button" class="min-h-9 shrink-0 px-1 text-red-600" :disabled="disabled" :aria-label="t('tickets.removeAttachment', { name: file.name })" @click="emit('update:modelValue', modelValue.filter((_, i) => i !== index))">{{ t('common.remove') }}</button>
      </li>
    </ul>
