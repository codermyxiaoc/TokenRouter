...
- 智能路由 Key 按一次重定向后的目标模型匹配候选账号目录，并选中首个有当前可调度账号且未冷却的分组。支持安全重放的请求在组内最终确认上游错误后依序换组，范围包含真实 HTTP 4xx/5xx、带内失败及明确上游网络错误，每轮重新鉴权；已输出业务内容、已有结算、本地校验或策略拒绝不重放。失败事件保留原分组，成功后另存恢复目标供管理员和本人错误页查看，详见[智能路由 API Key](../domains/smart_routing_api_keys.md#group_failover)。
...
Chat 兼容桥必须把 HTTP 200 中的流错误及未完成空流转换为失败事实，不能因为传输状态成功就安排正常结算。尚无业务输出或用量的可重试失败返回 `UpstreamFailoverError`；已输出或已有用量时返回明确失败终态与部分结果。智能路由的响应缓冲须在首次进入认证链前注册写入器恢复，即使首轮 handler panic，也要先恢复外层写入器，避免 Recovery 的错误响应滞留在无人提交的缓冲中。
